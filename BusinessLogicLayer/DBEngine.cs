...
namespace BusinessLogicLayer
{
    public class DBEngine
    {
        ...
        public string AuthenticateUser(string UserName, String Password)
        {
            ...
            if (Password != null)
            {
                whereClause += " and cast(user_password as varbinary) =cast('" + Password + "' as varbinary)" + " and cnt_internalid = user_contactid and emp_contactId=cnt_internalid and (emp_dateofLeaving is null or emp_dateofLeaving='1/1/1900 12:00:00 AM' OR emp_dateofLeaving>=getdate()) ";
            }
            ...
            string[,] ValidUser = GetFieldValue("tbl_master_user,tbl_master_contact,tbl_master_employee ",
                @"user_id,user_name,
            user_password,user_contactId, cnt_branchid, user_group,user_lastsegement,
            cnt_contactType,user_TimeForTickerRefrsh, cnt_shortName,user_superUser ,
            (select top 1 emp_Organization from tbl_trans_employeeCTC where emp_cntId=user_contactid
            and (emp_effectiveuntil is null or emp_effectiveuntil='1/1/1900 12:00:00 AM'
            OR emp_effectiveuntil>=getdate()) order by emp_effectiveDate desc) as emp_Organization,
            user_EntryProfile,user_status,user_lastIP,emp_id,user_AllowAccessIP,cnt_internalId,cnt_id,
            CONVERT (varchar(10),last_login_date, 105) AS Lastlogindate,CONVERT(varchar,last_login_date,8) As Lastlogintime,Isnull(emp_image,'') emp_image",
          ...
          // ............. Code Above Commented by Sam on 07112016 due to get user its own cnt_internalid....................
            ...
            else
            {
                ...
                else
                {
                    if (Password != null)
                    {
                        string[,] IsActive = GetFieldValue("tbl_master_user", "RTRIM(LTRIM(user_inactive))", "user_loginId ='" + UserName + "' and user_password='" + Password + "'", 1);
                        if (IsActive[0, 0].ToString() == "N")
                        {
                            ...
                            if (ValidUser[0, 10].ToString().Trim() == "Y")  //_must be treated as superuser: taking top level Hierarchy!
                            {
                                ...
                                //string DefaultBranchInLogin = cbl.GetSystemSettingsResult("DefaultBranchInLogin");
                                //if (DefaultBranchInLogin.ToUpper() == "YES")
                                //{
                            ...
                            #region Lastlogindate & image
                            HttpContext.Current.Session["Lastlogindate"] = ValidUser[0, 19].ToString();
                            HttpContext.Current.Session["Lastlogintime"] = ValidUser[0, 20].ToString();
                            HttpContext.Current.Session["UserImage"] = ValidUser[0, 21].ToString();
        ...
        {
            string branchs = "";
            DataTable DtSecond = GetDataTable(" SRV_EMPLOYEEBRANCHMAP ", " BranchId ", " EmployeeId= '" + EmployeeID + "'");
            if (DtSecond.Rows.Count != 0)
            {
        ...
        // Rev Sanchita [ Set Currency session variable in Cutoff login ]
        void SetCurrency()
        {
            

            if (HttpContext.Current.Session["LastCompany"] != null && HttpContext.Current.Session["UserSegID"] != null)
            {

                //string[,] dst_Tables2 = GetFieldValue(" TBL_MASTER_COMPANYEXCHANGE ",
                //                "EXCH_COMPID ", "COMPANYID ='" + HttpContext.Current.Session["LastCompany"].ToString() + "'", 2);

                BusinessLogicLayer.DBEngine oDBEngine = new BusinessLogicLayer.DBEngine("");
                DataTable dst_Tables2 = new DataTable();
                dst_Tables2 = oDBEngine.GetDataTable("(SELECT LTRIM(RTRIM(EXCH_COMPID)) AS COMPANYID FROM (SELECT EXCH_COMPID,EXCH_INTERNALID,EXH_SHORTNAME,EXCH_SEGMENTID FROM TBL_MASTER_EXCHANGE,TBL_MASTER_COMPANYEXCHANGE WHERE EXH_CNTID=EXCH_EXCHID) AS T1,MASTER_EXCHANGE WHERE EXCHANGE_SHORTNAME=EXH_SHORTNAME) AS T1", " COMPANYID ", "COMPANYID='" + HttpContext.Current.Session["LastCompany"].ToString() + "'");

                if (dst_Tables2 != null && dst_Tables2.Rows.Count > 0)
                {
                    //if (DTCompanyDetail.Rows[0]["Exh_ShortName"].ToString() != "NSDL" && DTCompanyDetail.Rows[0]["Exh_ShortName"].ToString() != "CDSL")
                    //{
                    if (HttpContext.Current.Session["ExchangeSegmentID"] != null)
                    {
                        if (HttpContext.Current.Session["LastCompany"].ToString() != "n" && HttpContext.Current.Session["ExchangeSegmentID"].ToString() != "n" && dst_Tables2.Rows.Count > 0)
                        {
                            //string[,] dst_Tables3_0 = GetFieldValue(" MASTER_EXCHANGESEGMENTS ",
                            //    "(SELECT CAST(CURRENCY_ID AS VARCHAR)+'~'+ LTRIM(RTRIM(CURRENCY_ALPHACODE))+'~'+LTRIM(RTRIM(REPLACE(CURRENCY_SYMBOL,CHAR(160),''))) AS TRADECURRENCY FROM MASTER_CURRENCY where CURRENCY_ID=MASTER_EXCHANGESEGMENTS.EXCHANGESEGMENT_TRADECURRENCYID ) AS TradeCurrency ", "EXCHANGESEGMENT_ID='" + HttpContext.Current.Session["ExchangeSegmentID"].ToString() + "'", 2);

                            DataTable dst_Tables3_0 = new DataTable();
                            dst_Tables3_0 = oDBEngine.GetDataTable("MASTER_EXCHANGESEGMENTS,MASTER_CURRENCY", " CAST(CURRENCY_ID AS VARCHAR)+'~'+ LTRIM(RTRIM(CURRENCY_ALPHACODE))+'~'+LTRIM(RTRIM(REPLACE(CURRENCY_SYMBOL,CHAR(160),''))) AS TRADECURRENCY ", "CURRENCY_ID=EXCHANGESEGMENT_TRADECURRENCYID AND EXCHANGESEGMENT_ID='" + HttpContext.Current.Session["ExchangeSegmentID"].ToString() + "'");


                            if (dst_Tables3_0 != null && dst_Tables3_0.Rows.Count > 0)  //
                            {
                                HttpContext.Current.Session["TradeCurrency"] = Convert.ToString(dst_Tables3_0.Rows[0]["TRADECURRENCY"]);
                                //First Time Active Currency Will Be Trade Currency ByDefault....
                                HttpContext.Current.Session["ActiveCurrency"] = Convert.ToString(dst_Tables3_0.Rows[0]["TRADECURRENCY"]);
                            }

                            //string[,] dst_Tables3_1 = GetFieldValue(" TBL_MASTER_COMPANY ",
                            //    "(SELECT CAST(CURRENCY_ID AS VARCHAR)+'~'+LTRIM(RTRIM(CURRENCY_ALPHACODE))+'~'+ LTRIM(RTRIM(REPLACE(CURRENCY_SYMBOL,CHAR(160),''))) FROM MASTER_CURRENCY where CURRENCY_ID=TBL_MASTER_COMPANY.CMP_CURRENCYID ) AS LOCALCURRENCY  ", 
                            //    "CMP_INTERNALID ='" + HttpContext.Current.Session["LastCompany"].ToString() + "'", 2);

                            DataTable dst_Tables3_1 = new DataTable();
                            dst_Tables3_1 = oDBEngine.GetDataTable("TBL_MASTER_COMPANY,MASTER_CURRENCY ", "CAST(CURRENCY_ID AS VARCHAR)+'~'+LTRIM(RTRIM(CURRENCY_ALPHACODE))+'~'+ LTRIM(RTRIM(REPLACE(CURRENCY_SYMBOL,CHAR(160),''))) AS LOCALCURRENCY", "CURRENCY_ID=CMP_CURRENCYID  AND CMP_INTERNALID='" + HttpContext.Current.Session["LastCompany"].ToString() + "'");

                            if (dst_Tables3_1 != null && dst_Tables3_1.Rows.Count > 0)  //
                            {
                                HttpContext.Current.Session["LocalCurrency"] = Convert.ToString(dst_Tables3_1.Rows[0]["LOCALCURRENCY"]);
                            }
        ...
        {
            HttpContext.Current.Session["TradeCurrency"] = null;
            HttpContext.Current.Session["LocalCurrency"] = null;
            HttpContext.Current.Session["ActiveCurrency"] = null;
        ...
        void CurrencySetting_NonExchangeSegment()
        {
            ...
            DTCurrency = oDBEngine.GetDataTable("Tbl_Master_Company,Master_Currency", @"Cast(Currency_ID as Varchar)+'~'+Ltrim(Rtrim(Currency_AlphaCode))+'~'+
                ...
                and cmp_InternalID='" + HttpContext.Current.Session["LastCompany"].ToString() + "'");

            if (DTCurrency.Rows.Count > 0)
            {
                HttpContext.Current.Session["TradeCurrency"] = DTCurrency.Rows[0][0].ToString();
                HttpContext.Current.Session["LocalCurrency"] = DTCurrency.Rows[0][0].ToString();
                HttpContext.Current.Session["ActiveCurrency"] = DTCurrency.Rows[0][0].ToString();
