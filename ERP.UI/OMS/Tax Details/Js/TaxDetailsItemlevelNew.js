...
CallServerToloadtax = function () {
    $.ajax({
        type: "POST",
        url: "/Services/TaxService.asmx/GetTaxDetailsForSaleException",
...
caluculateAndSetGST = function (AmountEditor, chargesEditor, TotalAmountEditor, HSNCode, Amount, AmountAfterDiscount, inclsOrExclsv, ShippingState, branchId, entitytype, date, qty, Module) {
    ...
    if (HSNObjectException.length > 0) {
        ...
        if (TaxTypeMode == "IGST") {
            totalTaxchargesRate = parseFloat(HSNObjectException[0].O_IGST);
        }
        else if (TaxTypeMode == "SGST") {
            totalTaxchargesRate = parseFloat(HSNObjectException[0].O_CGST) + parseFloat(HSNObjectException[0].O_SGST);
        }
        else if (TaxTypeMode == "UTGST") {
            totalTaxchargesRate = parseFloat(HSNObjectException[0].O_CGST) + parseFloat(HSNObjectException[0].O_UTGST);
        }
                ...
                if (TaxTypeMode == "IGST") {
                    if (taxes[taxCount].TaxTypeCode == "IGST") {
                        ...
                        if (taxes[taxCount].Taxes_ApplicableOn == "G") {
                            ...
                            if (inclsOrExclsv == "E") {
                                //Mantis Issue 24884
                               // TaxChargesAmount = DecimalRoundoff(parseFloat(TaxChargesAmount + (GrosstotalAmount * (taxes[taxCount].Rate / 100))).toFixed(2), 2);
                               //Rev 1.0
