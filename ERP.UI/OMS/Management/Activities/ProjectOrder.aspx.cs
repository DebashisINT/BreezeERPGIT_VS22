...
namespace ERP.OMS.Management.Activities
{
    public partial class ProjectOrder : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            string ApproveProjSalesOrder = ComBL.GetSystemSettingsResult("ApproveProjSalesOrder");
            ...
            string ProjectTermsCondition = ComBL.GetSystemSettingsResult("ShowProjectTCProjectSalesOrder");
            ...
            if (!IsPostBack)
            {
                Session["QuoteOrderDetails_id"] = null;
                Session["Entry_Type"] = null;
                Session["PO_ProductDetails"] = null;
                ...
                if (Request.QueryString["key"] != null)
                {
                    if (Convert.ToString(Request.QueryString["key"]) != "ADD")
                    {
                        ...
                        {
                            ltrTitle.Text = "Approve Project Sales Order";
                        ...
                        {
                            ltrTitle.Text = "Modify Project Sales Order";
                        }
                        ...
                        {
                            Doctype = "SO";
                            ProjectTermsConditionsControl.SetEditTermsCoditionData(Convert.ToString(strOrderId), Doctype);
                        }
                        //kaushik 25-2-2017
                    ...
                    else
                    {
                        ...
                        //lookup_quotation.DataBind();
                        // bindLookUP("NOT-DONE");
                        ltrTitle.Text = "Add Project Sales Order";
                        //  Session["STaxDetails"] = GetTaxData();
                ...
                {
                    ltrTitle.Text = "View Project Sales Order";
            ...
            if (Session["LastFinYear"] != null)
            {
                ...
                if (dtFinYear != null && dtFinYear.Rows.Count > 0)
                {
                    ...
                    if (Session["FinYearStartDate"] != null)
                    {
                        ...
                        //dtProjValidFrom.MinDate = Convert.ToDateTime(Convert.ToString(Session["FinYearStartDate"]));
                        //dtProjValidUpto.MinDate = Convert.ToDateTime(Convert.ToString(Session["FinYearStartDate"]));
                    }
                    if (Session["FinYearEndDate"] != null)
                    {
                        dt_PLSales.MaxDate = Convert.ToDateTime(Convert.ToString(Session["FinYearEndDate"]));
                        // dtProjValidFrom.MaxDate = Convert.ToDateTime(Convert.ToString(Session["FinYearEndDate"]));
                        // dtProjValidUpto.MaxDate = Convert.ToDateTime(Convert.ToString(Session["FinYearEndDate"]));
                    }
                }
            }
        ...
        public void GetAllDropDownDetailForSalesOrder(string UserBranch)
        {
            ...
            DataTable Schemadt = objSlaesActivitiesBL1.GetNumberingSchema(strCompanyID, userbranchHierarchy, FinYear, "111", "Y");
        ...
        public IEnumerable GetSalesOrder()
        {
            ...
            for (int i = 0; i < Orderdt.Rows.Count; i++)
            {
                ...
                Orders.BalQty = Convert.ToString(Orderdt.Rows[i]["BalQty"]);
            }
        ...
        public IEnumerable GetSalesOrder(DataTable SalesOrderdt)
        {
            ...
            for (int i = 0; i < SalesOrderdt.Rows.Count; i++)
            {
                ...
                //Orders.ProjectAdditionRemarks = Convert.ToString(SalesOrderdt.Rows[i]["ProjectAdditionRemarks"]);
                if (dtC.Contains("BalQty"))
                {
                    Orders.BalQty = Convert.ToString(SalesOrderdt.Rows[i]["BalQty"]);
                }
                OrderList.Add(Orders);
            }
        ...
        {
            List<ProjectQuotation> OrderList = new List<ProjectQuotation>();
            ...
            {
                ProjectQuotation Orders = new ProjectQuotation();
        ...
        public IEnumerable GetSalesOrderInfo(DataTable SalesOrderdt1, string Order_Id)
        {
            ...
            for (int i = 0; i < SalesOrderdt1.Rows.Count; i++)
            {
               ...
               // QuoteOrderDetails_id = strQuoteDetails_Id;
        ...
        public bool BindSessionByDatatable(DataTable dt)
        {
            ...
            SalesChalladt.Columns.Add("BalQty", typeof(string));

            for (int i = 0; i < dt.Rows.Count; i++)
            {
                IsSuccess = true;
                DataColumnCollection dtC = dt.Columns;
                string SalePrice, Discount, Amount, TaxAmount, TotalAmount, Order_Num1, ProductName, ProjectRemarks;
                ...
                SalesChalladt.Rows.Add(Convert.ToString(i + 1), Convert.ToString(i + 1), Convert.ToString(dt.Rows[i]["QuoteDetails_ProductId"]), Convert.ToString(dt.Rows[i]["QuoteDetails_ProductDescription"]),
                    Convert.ToString(dt.Rows[i]["QuoteDetails_Quantity"]), Convert.ToString(dt.Rows[i]["UOM_ShortName"]), "", Convert.ToString(dt.Rows[i]["QuoteDetails_StockQty"]), Convert.ToString(dt.Rows[i]["UOM_ShortName"]),
                               SalePrice, Discount, Amount, TaxAmount, TotalAmount, "U", Convert.ToInt64(dt.Rows[i]["Quotation_No"]), Order_Num1, ProductName, ProjectRemarks, BalQty);
            }
        ...
        public DataTable ProjectAddrDesc()
        {
            DataTable ds = new DataTable();
            ProcedureExecute proc = new ProcedureExecute("prc_ProjectOrder_Details");
            proc.AddVarcharPara("@Action", 500, "ProjectAddrDescDetails");
        ...
        protected void grid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            foreach (var args in e.InsertValues)
            {
                ...
                if (ProductDetails != "" && ProductDetails != "0")
                {
                    ...
                    SalesOrderdt.Rows.Add(SrlNo, Convert.ToString(InitVal), ProductDetails, Description, Quantity, UOM, Warehouse, StockQuantity, StockUOM, SalePrice, Discount, Amount, TaxAmount, TotalAmount, "I", QuotationNumber, ProductName, ProjectRemarks);
                    InitVal = InitVal + 1;
                }
            }
            ...
            foreach (var args in e.UpdateValues)
            {
                ...
                if (isDeleted == false)
                {
                    if (ProductDetails != "" && ProductDetails != "0")
                    {
                        ...
                        if (Isexists == false)
                        {
                            SalesOrderdt.Rows.Add(SrlNo, OrderID, ProductDetails, Description, Quantity, UOM, Warehouse, StockQuantity, StockUOM, SalePrice, Discount, Amount, TaxAmount, TotalAmount, "U", QuotationNumber, ProductName, ProjectRemarks);
                            //}
            ...
            if (IsDeleteFrom != "D")
            {
                try
                {
                    ...
                    //Code block comment start M
                    //antis Issue number 0018841     
                   ...
                   .Select(g => g.Key);
                    ...
                    if (tempSalesOrderdt.Rows.Count>0)
                    {
                        foreach (DataRow dr in tempSalesOrderdt.Rows)
                        {
                            ...
                            {
                                DataTable dtq = oDBEngine.GetDataTable("select (ISNULL(TotalQty,0)+isnull(BalanceQty,0)) TotQty from tbl_trans_SalesInquiryBalanceMap where  SalesInquirysDocId='" + Convert.ToInt32(dr["Quotation_No"]) + "' and ProductId='" + ProductID + "'");
                    ...
                    if (validate == "outrange" || validate == "duplicate" || validate == "checkWarehouse" ||
                        ...
                        || validate == "MoreThanStock" || validate == "ZeroStock" || validate == "Dormant_Customer" || validate == "ExceedQuantity")
                    {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            TaxDetailTable = gstTaxDetails.SetTaxTableDataWithProductSerialWithException(tempSalesOrderdt, "SrlNo", "ProductID",
                             "Amount", "TaxAmount", TaxDetailTable, "S", dt_PLSales.Date.ToString("yyyy-MM-dd"), strBranch, ShippingState, TaxType, hdnCustomerId.Value, "Quantity", "SQ");
                            ...
                            if (Convert.ToString(hdBasketId.Value).Trim() == "")
                            {
                                ...
                                {
                                    List<ERP.OMS.Management.Activities.ProjectOrder.ProductQuantity> obj = new List<ERP.OMS.Management.Activities.ProjectOrder.ProductQuantity>();
                                    obj = (List<ERP.OMS.Management.Activities.ProjectOrder.ProductQuantity>)HttpContext.Current.Session["SessionPackingDetails"];
                            ...
                            int id = (ModifySalesOrder(MainOrderID, strSchemeType, UniqueQuotation, strQuoteDate, strQuoteExpiry, strCustomer, strContactName, ProjId,
                                              ...
                                              , creditdays, strDueDate, IsInvenotry, IsFromCRM, PosForGst, SchemeList[0], duplicatedt2, ddlInventory.SelectedValue, projectValidFrom, projectValidUpto, ProjRemarks, ApproveRejectstatus, AppRejRemarks, RevisionDate, RevisionNo, Doctype
                                              , firstApprove));
                            if (id <= 0)
                            {
                                if (id == -1)
        ...
        protected void grid_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            else if (strSplitCommand == "BindGridOnQuotation")
            {
                ...
                QuoteOrderDetails_id = QuoteDetails_Id;
                Session["QuoteOrderDetails_id"] = QuoteDetails_Id;
                string Quote_Nos = Convert.ToString(e.Parameters.Split('~')[1]);
                string Type = rdl_Salesquotation.SelectedValue;
                string mode = "";
                if (rdl_Salesquotation.SelectedValue == "QN")
                {
                    mode = "GetQuotationDetailsFromProjectOrderDuplicateProduct";
                }
                else if (rdl_Salesquotation.SelectedValue == "SINQ")
                {
                ...
                if (Quote_Nos != "$")
                {
                    if (!string.IsNullOrEmpty(QuoComponent1))
                    {
                        ...
                        if (!string.IsNullOrEmpty(IdKey))
                        {
                            ...
                            {
                                if (Type == "QN")
                                    dt_QuotationDetails = objSlaesActivitiesBL.GetQuotationDetailsFromProjectOrderWithDuplicateProduct(QuoComponent1, QuoteDetails_Id, Product_id1, "Edit").Tables[0];
                                if (Type == "SINQ")
                                    dt_QuotationDetails = objSlaesActivitiesBL.GetInquiryDetailsFromSalesOrder(QuoComponent1, QuoteDetails_Id, Product_id1, "Edit").Tables[0];
                            }
                            else
                            {
                                if (Type == "QN")
                                {
                                    dt_QuotationDetails = objSlaesActivitiesBL.GetQuotationDetailsFromProjectOrderWithDuplicateProduct(QuoComponent1, QuoteDetails_Id, Product_id1, "Add").Tables[0];
                                    additionaldesc = objSlaesActivitiesBL.GetQuotationDetailsFromProjectOrderWithDuplicateProduct(QuoComponent1, QuoteDetails_Id, Product_id1, "Add").Tables[1];
                                }
                                else if(Type == "SINQ")
                                {
                                    dt_QuotationDetails = objSlaesActivitiesBL.GetInquiryDetailsFromSalesOrder(QuoComponent1, QuoteDetails_Id, Product_id1, "Add").Tables[0];
                                    additionaldesc = objSlaesActivitiesBL.GetInquiryDetailsFromSalesOrder(QuoComponent1, QuoteDetails_Id, Product_id1, "Add").Tables[1];
                                }
                            }
        ...
        //public static object LoadprojectAddDetails()
        //{
        //    List<ProjectAddDetails> listBank = new List<ProjectAddDetails>();
        //    if (HttpContext.Current.Session["userid"] != null)
        //    {
        //        DataTable ds = new DataTable();
        //        ProcedureExecute proc = new ProcedureExecute("prc_ProjectOrder_Details");
        ...
        //        listBank = (from DataRow dr in ds.Rows
        //                    select new ProjectAddDetails()
        //                    {
        //                        SrlNo = Convert.ToString(dr["SrlNo"]),
        //                        ProjectAdditionRemarks = Convert.ToString(dr["ProjectAdditionRemarks"])
        //                    }).ToList();
        //    }
        ...
        public int ModifySalesOrder(string OrderID, string strSchemeType, string strOrderNo, string strOrderDate, string strOrderExpiry, string strCustomer, string strContactName, Int64 ProjId,
                                    ...
                                    string CreditDays, string strDueDate, int IsInventory, int IsFromCRM, string PosForGst, string SchemaID, DataTable PackingDetailsdt, string Entry_type, string projectValidFrom, string projectValidUpto, string ProjRemarks, int ApproveRejectstatus, string AppRejRemarks, string RevisionDate, string RevisionNo,string Doctype
            , int firstApprove)
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("prc_ProjectOrder", con);
                ...
                if (hdnRevisionRequiredEveryAfterApproval.Value == "Yes")
                {
                    //if ((ApproveRejectstatus == 1) && (hdnPageStatForApprove.Value != "") && (hdnApprovalReqInq.Value == "1") && (firstApprove==1))
                    //{
                    //    cmd.Parameters.AddWithValue("@Action", "Add");
                    //}
                    //else if ((hdnApprovalReqInq.Value == "1") && (hdnisFirstApprove.Value == "Yes") && (hdnPageStatForApprove.Value != ""))
                    //{
                    //    cmd.Parameters.AddWithValue("@Action", "Add");
                    //}
                    //else
                    ...
                    if (ApproveRejectstatus == 1 && hdnisFirstApprove.Value == "Yes" && hdnIsfromApproval.Value=="Yes")
                    {
                        cmd.Parameters.AddWithValue("@Action", "Add");
                    }
                    else
                ...
                if (idFromString > 0)
                {
                    ...
                    //Chinmoy commentted start 25-11-2019
                    ...
                    //Chinmoy commentted End 25-11-2019
                    if (hdnProjectSOTC.Value == "1")
                    {
                        ProjectTermsConditionsControl.Save_ProjectTerms(Convert.ToString(idFromString), "SO");
                    }
                }
        ...
        protected void grid_Products_DataBinding(Object sender, EventArgs e)
        {
            ...
            if (Session["PO_ProductDetails"] != null)
            {
                DataTable Quotationdt = (DataTable)Session["PO_ProductDetails"];
                DataView dvData = new DataView(Quotationdt);
                //dvData.RowFilter = "Status <> 'D'";
        ...
        protected void cgridProducts_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            if (strSplitCommand == "BindProductsDetails")
            {
                ...
                if (Quote_Nos != "$")
                {
                    ...
                    //grid.DataSource = GetSalesOrderInfo(dt_QuotationDetails, IdKey);
                    //grid.DataBind();
                    Session["PO_ProductDetails"] = dt_QuotationDetails;
        ...
        protected void acbpCrpUdf_Callback(object source, DevExpress.Web.CallbackEventArgsBase e)
        {
            ...
            if (Request.QueryString["key"] == "ADD")
            {
                ...
                if (DT_TC != null && DT_TC.Rows.Count > 0)
                {
                    ...
                    if (Convert.ToString(DTVisible.Rows[0]["Variable_Value"]).Trim() == "Yes")
                    {  
                        //Chinmoy commented below line start
                        if (IsMandatory == "Yes")
                        {
                        ...
                        //Chinmoy commented below line end
                    }
                    else { acbpCrpUdf.JSProperties["cpTC"] = "true"; }
                }
                #endregion
        ...
        public class ProjectAddDetails
        {
        ...
        public void SetOrderDetails()
        {
            ...
            if (OrderEditdt != null && OrderEditdt.Rows.Count > 0)
            {
                ...
                string isLastEntry = Convert.ToString(OrderEditdt.Rows[0]["isLastEntry"]);
                ...
                hdnisFirstApprove.Value = Convert.ToString(OrderEditdt.Rows[0]["isFirstApprove"]); ;

            }


            DataTable taginvoiceCheck = oDBEngine.GetDataTable("select 1 as Id from tbl_trans_SalesInvoice where  InvoiceCreatedFromDoc_Ids in ('" + Convert.ToString(Session["OrderID"]) + "')");

            if (taginvoiceCheck != null && taginvoiceCheck.Rows.Count>0)
            {
        ...
        protected void ComponentDatePanel_Callback(object sender, CallbackEventArgsBase e)
        {
            ...
            if (strSplitCommand == "BindQuotationDate")
            {
                ...
                {
                 dt_QuotationDetails=   objSlaesActivitiesBL.GetSalesInquiryDate(Quote_No);
                }
        ...
        protected void BindLookUp(String customer, string OrderDate,  string Doctype)
        {
            ...
            QuotationTable = objBL.GetDocOnProjectSalesOrder(customer, OrderDate, status,
        ...
        protected void ComponentQuotation_Callback(object source, DevExpress.Web.CallbackEventArgsBase e)
        {
            ...
            if (e.Parameter.Split('~')[0] == "BindQuotationGrid")
            {
                ...
                QuotationTable = objBL.GetDocOnProjectSalesOrder(customer, OrderDate, status, Convert.ToInt32(ddl_Branch.SelectedValue), Convert.ToString(ddlInventory.SelectedValue), Type);
                lookup_quotation.GridView.Selection.CancelSelection();
                lookup_quotation.DataSource = QuotationTable;
                lookup_quotation.DataBind();
                    ...
                    if (!String.IsNullOrEmpty(QuotationIds) && QuotationIds !="0")
                    {
                        ...
                        if (eachQuo.Length > 1)//More tha one quotation
                        {
                           // dt_Quotation.Text = "Multiple Select Quotation Dates";
                            ...
                            if (rdl_Salesquotation.SelectedValue == "SINQ")
                            {
                                rdl_Salesquotation.SelectedValue = "SINQ";
                                dt_Quotation.Text = "Multiple Select Inquiry Dates";
                            }
        ...
        public void DeleteUnsaveaddRemarks(string SrlNo, string RemarksVal)
        {
            DataTable Warehousedt = new DataTable();
            if (Session["InlineRemarks"] != null)
            {
                Warehousedt = (DataTable)Session["InlineRemarks"];

                var rows = Warehousedt.Select("SrlNo ='" + SrlNo + "' AND ProjectAdditionRemarks='" + RemarksVal + "'");
                foreach (var row in rows)
                {
                    row.Delete();
                }
                ...
                Session["InlineRemarks"] = Warehousedt;
            }
        }
        ...
        protected void GrdWarehouse_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            string strSplitCommand = e.Parameters.Split('~')[0];
            string Type = "";
            GrdWarehouse.JSProperties["cpWarehouseFinalOK"] = "";
            if (strSplitCommand == "Display")
            {
                GetProductType(ref Type);
            ...
            else if (strSplitCommand == "WarehouseFinal")
            {
                if (Session["SalesWarehouseData"] != null)
                {
                    ...
                    Session["SalesWarehouseData"] = Warehousedt;
                    GrdWarehouse.JSProperties["cpWarehouseFinalOK"] = "WarehouseFinalOK";
                }
            }
        }
        ...
        [WebMethod]
        public static int RevisionNumberCheck(string RevNo, string Order)
        {
            BusinessLogicLayer.DBEngine oDBEngine = new BusinessLogicLayer.DBEngine();
            int returnVal = 0;
            CRMSalesOrderDtlBL objCRMSalesOrderDtlBL = new CRMSalesOrderDtlBL();
            ...
            OrderNumber = oDBEngine.GetDataTable("select ISNULL(Order_Number,'') Order_Number from tbl_trans_SalesOrder  where Order_Id='" + Order + "'");
            dtrev = objCRMSalesOrderDtlBL.DuplicateRevisionNumber(RevNo, Convert.ToString(OrderNumber.Rows[0]["Order_Number"]));
            if (Convert.ToInt32(dtrev.Rows[0]["Order_RevisionNo"]) > 0)
            {
                returnVal = 1;
            }
            return returnVal;
        ...
        public static object GetApproveRejectStatus(string OrderId)
        {
            ...
            DataTable dts = new DataTable();
            CRMSalesOrderDtlBL objCRMSalesOrderDtlBL = new CRMSalesOrderDtlBL();
            dts = objCRMSalesOrderDtlBL.ApproveRejectProjectStatus(OrderId);
