...
{
    public partial class RateDifferenceEntryCustomer : System.Web.UI.Page
    {
        ...
        {
            rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSession("/management/Activities/RateDifferenceEntryCustomer.aspx");
            ...
            if (!IsPostBack)
            {
                ...
                string BackDatedEntryPurchaseReturnManual = ComBL.GetSystemSettingsResult("BackDatedEntryRDEC");
                ...
                this.Session["LastCompanyRDEC"] = Session["LastCompany"];
                this.Session["LastFinYearRDEC"] = Session["LastFinYear"];
                chk_Isclosed.Checked = true;
                //18-10-2017
                // GetProductData();
                //18-10-2017
                ...
                Session["RDEC_BillingAddressLookup"] = null;
                Session["RDEC_ShippingAddressLookup"] = null;
                #endregion Session Null Initialization End
                //Purpose : Binding Batch Edit Grid
                //Name : Sudip 
                // Dated : 21-01-2017
                Session["SecondHandsaleRDECExist"] = null;
                Session["RDEC_ReturnID"] = "";
                Session["RDEC_CustomerDetail"] = null;
                Session["RDEC_ReturnDetails"] = null;
                Session["RDEC_WarehouseData"] = null;
                Session["RDEC_QuotationTaxDetails"] = null;
                Session["LoopSRWarehouse"] = 1;
                Session["RDEC_TaxDetails"] = null;

                Session["RDEC_InvoiceTaxDetails"] = null;
                Session["RDEC_ActionType"] = "";
                Session["RDEC_ComponentData"] = null;
                Session["TaggingRDEVInvoce"] = "";
                ...
                Session["RDEC_QuotationAddressDtl"] = null;
                ...
                if (Request.QueryString["key"] != null)
                {
                    if (Convert.ToString(Request.QueryString["key"]) != "ADD")
                    {
                        ...
                        lblHeadTitle.Text = "Rate Difference Entry Customer";
                        hdnPageStatus.Value = "update";
                        divScheme.Style.Add("display", "none");
                        strSalesReturnId = Convert.ToString(Request.QueryString["key"]);
                        Session["RDEC_KeyVal_InternalID"] = "SRQUOTE" + strSalesReturnId;

                        rdl_SalesInvoice.Enabled = false;
                        rdl_SalesInvoice.SelectedValue = "SIN";
                        Keyval_internalId.Value = "RateDifferenceEntryCustomer" + strSalesReturnId;
                        #region Subhra Section
                        Session["RDEC_key_QutId"] = strSalesReturnId;
                        ...
                        Session["RDEC_ReturnID"] = strSalesReturnId;
                        Session["RDEC_ActionType"] = "Edit";
                        SetSalesReturnDetails(strSalesReturnId);
                        Session["RDEC_TaxDetails"] = GetTaxDataWithGST(GetTaxData(dt_PLQuote.Date.ToString("yyyy-MM-dd")));

                        DataTable Productdt = new DataTable();
                        DataTable Closed_dt = new DataTable();
                        DataSet ProducDts = new DataSet();


                        if (Convert.ToString(Session["RDECDocType"]) == "TSI")
                        {
                            ProducDts = objSalesReturnBL.GetTransitSalesInvoiceProductData(strSalesReturnId, Convert.ToString(Session["LastFinYear"]), Convert.ToString(Session["LastCompany"]));
                        }
                        else
                        {
                            ProducDts = objSalesReturnBL.GetSalesInvoiceProductData(strSalesReturnId, Convert.ToString(Session["LastFinYear"]), Convert.ToString(Session["LastCompany"]));
                        }
                        Productdt = ProducDts.Tables[0];

                        Closed_dt = ProducDts.Tables[1];
                        ...
                        //end rev rajdip
                        Session["RDEC_ReturnDetails"] = Productdt;
                        grid.DataSource = GetQuotation(Productdt);
                        grid.DataBind();

                        if (Closed_dt != null && Closed_dt.Rows.Count > 0)
                        {
                            //ASPxListBox list = ASPxDropDownEdit1.FindControl("listBox") as ASPxListBox;
                            string Isclose = Convert.ToString(Closed_dt.Rows[0]["IsClosed"]).ToUpper();
                            if (Isclose == "TRUE")
                            {
                                chk_Isclosed.Checked = true;
                            }
                            ...
                            {
                                chk_Isclosed.Checked = false;
                            }
                        }

                        Session["RDEC_QuotationAddressDtl"] = objSalesReturnBL.GetSalesReturnBillingAddress(strSalesReturnId); ;
                        ...
                        if (GetQuotationTaxData().Tables[0] != null)
                            Session["RDEC_QuotationTaxDetails"] = GetQuotationTaxData().Tables[0];
                        ...
                        if (GetQuotationEditedTaxData().Tables[0] != null)
                        {
                            ...
                            {
                                Session["RDEC_FinalTaxRecord"] = quotetable;
                            }
                        }
                        ...
                        {
                            lblHeadTitle.Text = "View  Rate Difference Entry Customer";
                    ...
                    else
                    {
                        ...
                        if (Session["RDEC_SaveMode"] != null)  // it has been removed from coding side of Quotation list 
                        {
                            if (Session["RDEC_schemavalue"] != null)  // it has been removed from coding side of Quotation list 
                            {
                                ddl_numberingScheme.SelectedValue = Convert.ToString(Session["RDEC_schemavalue"]); // it has been removed from coding side of Quotation list 
                            }
                            if (Convert.ToString(Session["RDEC_SaveMode"]) == "A")
                            {
                                dt_PLQuote.Focus();
                                txt_PLQuoteNo.Enabled = false;
                                txt_PLQuoteNo.Text = "Auto";
                            }
                            else if (Convert.ToString(Session["RDEC_SaveMode"]) == "M")
                            {
                        ...
                        Session["RDEC_ActionType"] = "Add";
                        //ASPxButton2.Enabled = false;
                        Session["RDEC_TaxDetails"] = null;
                        CreateDataTaxTable();
                        lblHeadTitle.Text = "Add  Rate Difference Entry Customer";
        ...
        public DataSet GetQuotationData()
        {
            ...
            proc.AddVarcharPara("@QuotationID", 500, Convert.ToString(Session["RDEC_ReturnID"]));
            ds = proc.GetDataSet();
            return ds;
        }
        protected void grid_CellEditorInitialize(object sender, ASPxGridViewEditorEventArgs e)
        {
            ...
            {
                if (Convert.ToString(Session["SecondHandsaleRDECExist"]) == "1")
                {
        ...
        protected void grid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (Session["RDEC_ReturnDetails"] != null)
            {
                Quotationdt = (DataTable)Session["RDEC_ReturnDetails"];
            }
            else
            {
            ...
            foreach (DataRow dr in Quotationdt.Rows)
            {
                ...
                string ProductDetails = Convert.ToString(dr["ProductDisID"]);
                string[] ProductDetailsList = ProductDetails.Split(new string[] { "||@||" }, StringSplitOptions.None);
                string ProductDisID = Convert.ToString(ProductDetailsList[0]);
                dr["ProductDisID"] = ProductDisID;
            ...
            Session["RDEC_ReturnDetails"] = Quotationdt;
            ...
            if (IsDeleteFrom != "D")
            {
                ...
                string ActionType = Convert.ToString(Session["RDEC_ActionType"]);
                string MainInvoiceID = Convert.ToString(Session["RDEC_ReturnID"]);
                ...
                string strComponenyType = Convert.ToString(rdl_SalesInvoice.SelectedValue);
                ...
                if (Session["RDEC_FinalTaxRecord"] != null)
                {
                    TaxDetailTable = (DataTable)Session["RDEC_FinalTaxRecord"];
                }

                // DataTable of Warehouse

                DataTable tempWarehousedt = new DataTable();
                if (Session["RDEC_WarehouseData"] != null)
                {
                    DataTable Warehousedt = (DataTable)Session["RDEC_WarehouseData"];
                ...
                if (Session["RDEC_TaxDetails"] != null)
                {
                    TaxDetailsdt = (DataTable)Session["RDEC_TaxDetails"];
                }
                else
                {
                ...
                #region #### Old_Process ####
                ////DataTable tempBillAddress = new DataTable();
                ////if (Session["RDEC_QuotationAddressDtl"] != null)
                ////{
                ////    tempBillAddress = (DataTable)Session["RDEC_QuotationAddressDtl"];
                ////}
                ////else
                ////{
               ...
               .Select(g => g.Key);
                ...
                if (!String.IsNullOrEmpty(GSTRateTaxMasterMandatory))
                {
                    if (GSTRateTaxMasterMandatory == "Yes")
                    {
                        ...
                        procT.AddVarcharPara("@Action", 500, "GetTaxDetailsByProductIDForRDEC");
                        procT.AddPara("@RDECProductDetails", tempQuotation);
                ...
                if (validate == "outrange" || validate == "duplicate" || validate == "checkWarehouse" || validate == "duplicateProduct" || validate == "nullAmount" || validate == "nullQuantity" || validate == "checkAddress" || validate == "checkAcurateTaxAmount")
                {
                ...
                else
                {
                    ...
                    if (Convert.ToString(Session["RDEC_ActionType"]) == "Add") // session has been removed from quotation list page working good
                    {
                        ...
                        Session["RDEC_schemavalue"] = schemavalue;        // session has been removed from quotation list page working good
                        ...
                        if (hdnRefreshType.Value == "N")
                        {
                            if (schematype == "1")
                            {
                                Session["RDEC_SaveMode"] = "A";
                            }
                            else
                            {
                                Session["RDEC_SaveMode"] = "M";
                            }
                        }
                    }
                    ...
                    string ShippingStateValuefortax = "0";

                    if (hdnPlaceOfSupply.Value != null)
                        ShippingStateValuefortax = hdnPlaceOfSupply.Value.Split('~')[1].ToString();

                    string strTaxTypesTax = "";
                    if (ddl_AmountAre.Value.ToString() == "1")
                        strTaxTypesTax = "E";
                    else if (ddl_AmountAre.Value.ToString() == "2")
                        strTaxTypesTax = "I";
        ...
        protected void grid_DataBinding(object sender, EventArgs e)
        {
            ...
            if (Session["RDEC_ReturnDetails"] != null)
            {
                DataTable Quotationdt = (DataTable)Session["RDEC_ReturnDetails"];
        ...
        protected void grid_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            else if (strSplitCommand == "RemoveDisplay")
            {
                ...
                if (Session["RDEC_ReturnDetails"] != null)
                {
                    salesReturndt = (DataTable)Session["RDEC_ReturnDetails"];
                }
                ...
                Session["RDEC_ReturnDetails"] = salesReturndt;
                grid.DataBind();
                ...
                if (Quote_Nos != "$")
                {
                    ...
                    if (!string.IsNullOrEmpty(IdKey))
                    {
                        if (IdKey != "ADD")
                        {
                            if (SelectedSalesInvoiceType != "TSI")
                            {
                                dt_QuotationDetails1 = objSalesReturnBL.GetInvoiceDetailsOnly(QuoComponent1, "Edit");
                                dt_SecondHandSaleDetails = objSalesReturnBL.GetInvoiceDetailsForSecondHandSale(QuoComponent1);
                                Session["SecondHandsaleRDECExist"] = Convert.ToString(dt_SecondHandSaleDetails.Rows[0]["IsExistsSeconHandSale"]);
                            }
                        }
                        else
                        {
                            if (SelectedSalesInvoiceType != "TSI")
                            {
                                dt_QuotationDetails1 = objSalesReturnBL.GetInvoiceDetailsOnly(QuoComponent1, "Add");
                                dt_SecondHandSaleDetails = objSalesReturnBL.GetInvoiceDetailsForSecondHandSale(QuoComponent1);
                                Session["SecondHandsaleRDECExist"] = Convert.ToString(dt_SecondHandSaleDetails.Rows[0]["IsExistsSeconHandSale"]);
                            }

                            DataTable dt = objSalesReturnBL.GetNecessaryData(QuoComponent1, SelectedSalesInvoiceType);
                            if (dt != null && dt.Rows.Count > 0)
                            {
                            ...
                            grid.JSProperties["cpInvoiceDetails"] = Reference + "~" + Currency_Id + "~" + SalesmanId + "~" + ExpiryDate + "~" + CurrencyRate + "~" + Contact_person_id + "~" + Tax_option + "~" + Tax_Code;
                    ...
                    if (dt_QuotationDetails1 != null && dt_QuotationDetails1.Rows.Count > 0)
                    {
                    ...
                    using (var client = new HttpClient())
                    {
                        ...
                        else
                        {
                            ...
                            if (err.error.type != "ClientRequest")
                            {
                                ...
                                {
                                    objDB.GetDataTable("INSERT INTO EInvoice_ErrorLog(DOC_ID,DOC_TYPE,ERROR_TYPE,ERROR_CODE,ERROR_MSG) VALUES ('" + id.ToString() + "','SR','IRN_GEN','" + item.ErrorCode + "','" + item.ErrorMessage + "')");
                                }
                            }
                            else
                            {
                                    ...
                                    ref int strIsComplete, ref int strInvoiceID, string ReasonforReturn, Int64 Proj_Id, string drdTransCategory)
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("prc_CRMRateDiffEntryCust", con);
                ...
                cmd.Parameters.AddWithValue("@ComponentType", "RDEC");
                ...
                bool isSRClosed = false;
                if (chk_Isclosed.Checked)
                {
                    isSRClosed = true;
                }
        ...
        public DataTable GetTaxData(string quoteDate)
        {
            ...
            proc.AddVarcharPara("@SalesReturnID", 500, Convert.ToString(Session["RDEC_ReturnID"]));
            proc.AddVarcharPara("@FinYear", 500, Convert.ToString(Session["LastFinYear"]));
            // proc.AddVarcharPara("@branchId", 500, Convert.ToString(Session["userbranchID"]));
        ...
        protected void gridTax_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            if (strSplitCommand == "Display")
            {
             //   Session["RDEC_TaxDetails"] = null;
                ...
                if (Session["RDEC_TaxDetails"] == null)
                {
                    Session["RDEC_TaxDetails"] = GetTaxData(dt_PLQuote.Date.ToString("yyyy-MM-dd"));
                }
            ...
            else if (strSplitCommand == "SaveGst")
            {
                ...
                else
                {
                    ...
                    Session["RDEC_TaxDetails"] = TaxDetailsdt;
        ...
        protected void gridTax_DataBinding(object sender, EventArgs e)
        {
            if (Session["RDEC_TaxDetails"] != null)
            {
                DataTable TaxDetailsdt = (DataTable)Session["RDEC_TaxDetails"];
        ...
        protected void GrdWarehousePC_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            if (strSplitCommand == "Display")
            {
                ...
                else
                {
                    ...
                    if (Session["RDEC_WarehouseData"] != null)
                    {
                        Warehousedt = (DataTable)Session["RDEC_WarehouseData"];
                    }
                    else
                    {
                    ...
                    if (Session["RDEC_WarehouseData"] != null)
                    {
                        DataTable Warehousedts = (DataTable)Session["RDEC_WarehouseData"];
                    ...
                    else
                    {
                        ...
                        if (inputqnty <= totalopeining || isedited == "true")
                        {
                            ...
                            Session["RDEC_WarehouseData"] = Warehousedt;
            ...
            {
                if (Session["RDEC_WarehouseData"] == null && hdnisolddeleted.Value == "false")
                {
                ...
                else if (Session["RDEC_WarehouseData"] == null && hdnisolddeleted.Value == "true")
                {
            ...
            if (strSplitCommand == "Delete")
            {
                ...
                if (Session["RDEC_WarehouseData"] != null)
                {
                    ...
                    if (dr.Count() > 0)
                    {
                        ...
                        GrdWarehousePC.DataSource = Session["RDEC_WarehouseData"];
                        GrdWarehousePC.DataBind();
                    }
                    else
                    {
        ...
        public DataTable GetRecord(string stockids)
        {
            ...
            if (dts.Rows.Count > 0)
            {
                ...
                {
                    if (!string.IsNullOrEmpty(Convert.ToString(dts.Rows[i]["BatchWarehouseID"])) && Convert.ToString(dts.Rows[i]["BatchWarehouseID"]) != "0" && Convert.ToString(Session["RDEC_ActionType"]) == "Edit")
                    {
                        isoldornew = "old";
                    }
                    else
        ...
        private int Insertupdatedata(string ProductID, string stockid, string branchid)
        {
            ...
            {
                DataTable dt = new DataTable();
                DataTable Warehousedtups = (DataTable)Session["RDEC_WarehouseData"];
        ...
        public DataTable GetStockWarehouseData(string stockid)
        {
            ...
            if (Convert.ToString(Session["RDEC_ActionType"]) == "Edit")
            {
                DataSet dsInst = new DataSet();
                ...
                cmd.Parameters.AddWithValue("@Finyear", Convert.ToString(Session["LastFinYear"]));
                cmd.Parameters.AddWithValue("@PCNumber", Convert.ToString(Session["RDEC_ReturnID"]));
            ...
            else if (Convert.ToString(Session["RDEC_ActionType"]) != "Edit")
            {
                DataSet dsInst = new DataSet();
        ...
        protected void taxUpdatePanel_Callback(object source, DevExpress.Web.CallbackEventArgsBase e)
        {
            ...
            {
                DataTable MainTaxDataTable = (DataTable)Session["RDEC_FinalTaxRecord"];
                ...
                Session["RDEC_FinalTaxRecord"] = MainTaxDataTable;
                GetStock(Convert.ToString(performpara.Split('~')[1]));
                // DeleteWarehouse(Convert.ToString(performpara.Split('~')[1]));
                DataTable taxDetails = (DataTable)Session["RDEC_TaxDetails"];
                if (taxDetails != null)
                {
                    ...
                    Session["RDEC_TaxDetails"] = taxDetails;
                }
            }
            ...
            else
            {
                ...
                if (deletedRow.Length > 0)
                {
                    ...
                    {
                        //chinmoy added
                        if (Convert.ToString(dr["Amount"]) == "0")
                            dr["Amount"] = "0.00";
                    }
        ...
        public void CreateDataTaxTable()
        {
            ...
            Session["RDEC_FinalTaxRecord"] = TaxRecord;
        }
        public static void CreateDataTaxTableAjax()
        {
            ...
            TaxRecord.Columns.Add("Percentage", typeof(System.Decimal));
            TaxRecord.Columns.Add("Amount", typeof(System.Decimal));
            HttpContext.Current.Session["RDEC_FinalTaxRecord"] = TaxRecord;
        }
        ...
        protected void cgridTax_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            {
                DataTable TaxRecord = (DataTable)Session["RDEC_FinalTaxRecord"];
            ...
            else
            {
                ...
                ProcedureExecute pro = new ProcedureExecute("prc_PosSalesCRM_Details");
                pro.AddVarcharPara("@Action", 500, "LoadOtherTaxDetails");
                pro.AddVarcharPara("@ProductID", 10, Convert.ToString(setCurrentProdCode.Value));
                pro.AddVarcharPara("@S_quoteDate", 10, dt_PLQuote.Date.ToString("yyyy-MM-dd"));
                taxDetail = pro.GetTable();
                ...
                var cid = Convert.ToString(e.Parameters.Split('~')[2]);
                var IsChangePrice = Convert.ToString(e.Parameters.Split('~')[3]);
                int slNo = Convert.ToInt32(HdSerialNo.Value);
                ...
                else
                {
                    ...
                    //if (IsChangePrice == "N")
                    //{ TaxRecord = (DataTable)Session["RDEC_FinalTaxRecord"]; }
                    //else
                    //{
                        //  ImplementTaxonTaggingTRDEV(Convert.ToInt32(cid), Convert.ToInt32(slNo));
                    ...
                    foreach (DataRow dr in taxDetail.Rows)
                    {
                        ...
                        //        continue;
                          ...
                          //  }
                            ...
                            if (Convert.ToDecimal(obj.TaxField) != 0)
                            {
                                obj.Amount = Convert.ToDouble(obj.calCulatedOn * (Convert.ToDecimal(obj.TaxField) / 100));
                            }
        ...
        public void DeleteTaxDetails(string SrlNo)
        {
            ...
            if (Session["RDEC_FinalTaxRecord"] != null)
            {
                ...
                Session["RDEC_FinalTaxRecord"] = TaxDetailTable;
            }
        }
        ...
        public static bool CheckUniqueCode(string ReturnNo)
        {
            ...
            try
            {
                ...
                flag = objShortNameChecking.CheckUnique(ReturnNo, "0", "RateDifferenceEntryCustomer");
            }
            catch (Exception ex)
            {
        ...
        public void GetAllDropDownDetailForSalesQuotation(string userbranch)
        {
            ...
            SlaesActivitiesBL objSlaesActivitiesBL = new SlaesActivitiesBL();
            DataTable Schemadt = objSlaesActivitiesBL.GetNumberingSchema(strCompanyID, userbranchHierarchy, FinYear, "58", "Y");
        ...
        //{
        //    if (Session["RDEC_CustomerDetail"] == null)
        //    {
        ...
        //            Session["RDEC_CustomerDetail"] = dtCustomer;
        //        }
        //    }
        //    else
        //    {
        //        lookup_Customer.DataSource = (DataTable)Session["RDEC_CustomerDetail"];
        //        lookup_Customer.DataBind();
        //    }
        ...
        public static object GetContactPerson(string VendorId)
        {
            ...
            {
                DataTable dtContactPerson = new DataTable();
                dtContactPerson = objSlaesActivitiesBL.PopulateContactPersonOfCustomer(VendorId);
                if (dtContactPerson != null && dtContactPerson.Rows.Count > 0)
                {
        ...
        public DataTable GetProjectEditData(string strSalesReturnId)
        {
            DataTable dt = new DataTable();
            ProcedureExecute proc = new ProcedureExecute("prc_CRMSalesReturn_Details");
            proc.AddVarcharPara("@Action", 500, "RDECProjectEditdata");
            proc.AddVarcharPara("@SalesReturnID", 200, strSalesReturnId);
            dt = proc.GetTable();
            return dt;
        }
        public void SetSalesReturnDetails(string strSalesReturnId)
        {
            DataTable SalesReturnEditdt = objSalesReturnBL.GetSalesReturnEditData(strSalesReturnId);
            DataTable PosData = objSalesReturnBL.SalesReturnDetailsForEdit(strSalesReturnId).Tables[2];
            if (SalesReturnEditdt != null && SalesReturnEditdt.Rows.Count > 0)
            {
                ...
                string DocType = Convert.ToString(SalesReturnEditdt.Rows[0]["DType"]);
                ...
                string placeOfSupply = Convert.ToString(PosData.Rows[0]["Name"]);
                int placeOfSupplyStateId = Convert.ToInt32(PosData.Rows[0]["StateId"]);
                string StateCode = Convert.ToString(PosData.Rows[0]["StateCode"]);

                ddlPosGstRDEC.DataSource = PosData;

                ddlPosGstRDEC.ValueField = "StateId";
                ddlPosGstRDEC.TextField = "Name";
                ddlPosGstRDEC.DataBind();

                string PosForGst = Convert.ToString(SalesReturnEditdt.Rows[0]["PosForGst"]);
                ddlPosGstRDEC.Value = placeOfSupply;
                ...
                Session["RDECDocType"] = DocType;
                if (DocType != "")
                {
                    rdl_SalesInvoice.SelectedValue = DocType;
                ...
                if (!String.IsNullOrEmpty(InvoiceCreatedFromDoc_Ids))
                {
                    ...
                    if (eachQuo.Length > 1)//More tha one quotation
                    {
                        BindLookUp(Customer_Id, Convert.ToDateTime(Quote_Date).ToString("yyyy-MM-dd"), Branch_Id, DocType);
                        foreach (string val in eachQuo)
                        {
                            lookup_quotation.GridView.Selection.SelectRowByKey(Convert.ToInt32(val));
                        }
                        Session["TaggingRDEVInvoce"] = "1";
                    }
                    else if (eachQuo.Length == 1)//Single Quotation
                    {
                        BindLookUp(Customer_Id, Convert.ToDateTime(Quote_Date).ToString("yyyy-MM-dd"), Branch_Id, DocType);
        ...
        protected void ComponentQuotation_Callback(object source, DevExpress.Web.CallbackEventArgsBase e)
        {
            ...
            if (e.Parameter.Split('~')[0] == "BindComponentGrid")
            {
                ...
                string strReturnID = Convert.ToString(Session["RDEC_ReturnID"]);
                ...
                if (e.Parameter.Split('~')[4] == "SI")
                {
                    ComponentTable = objSalesReturnBL.GetInvoiceRDEC(Customer, SalesReturnDate, strReturnID, branchId);


                }
                if (e.Parameter.Split('~')[4] == "TSI")
                {
                    ComponentTable = objSalesReturnBL.GetTransitInvoiceRDEC(Customer, SalesReturnDate, strReturnID, branchId);
                }
                lookup_quotation.GridView.Selection.CancelSelection();
                lookup_quotation.DataSource = ComponentTable;
                lookup_quotation.DataBind();
                Session["RDEC_ComponentData"] = ComponentTable;
        ...
        {
            if (Session["RDEC_ComponentData"] != null)
            {
                lookup_quotation.DataSource = (DataTable)Session["RDEC_ComponentData"];
            }
        }
        ...
        protected void ComponentDatePanel_Callback(object sender, CallbackEventArgsBase e)
        {
            ...
            if (strSplitCommand == "BindComponentDate")
            {
                ...
                string Type = rdl_SalesInvoice.SelectedValue;
                if (Type == "SI")
                {
                    Dt = objSalesReturnBL.GetInvoiceDate(Invoice_No);
                }
                else if (Type == "TSI")
                {
                    Dt = objSalesReturnBL.GetTransitInvoiceDate(Invoice_No);
                }


                if (Dt != null && Dt.Rows.Count > 0)
                {
                    string invoicedate = Convert.ToString(Dt.Rows[0]["Invoice_Date"]);
                    if (!string.IsNullOrEmpty(invoicedate))
                    {
                        txt_InvoiceDate.Text = Convert.ToString(invoicedate);
                    }
                }

                DataTable invoicenumber = objSalesReturnBL.GetSalesTransitInvoiceNumber(Invoice_No, Type);
        ...
        protected void cgridProducts_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            if (strSplitCommand == "BindProductsDetails")
            {
                ...
                string Quote_Type = Convert.ToString(e.Parameters.Split('~')[2]);
                ...
                if (KeyVal != " ")
                {
                    ...
                    if (Quote_Type == "SI")
                    {
                        ...
                        if (!string.IsNullOrEmpty(IdKey))
                        {
                            if (IdKey != "ADD")
                            {
                                dt_QuotationDetails = objSalesReturnBL.GetsalesInvoiceDetailsOnly(KeyVal, "Edit");
                                // dt_QuotationDetails = objPurchaseChallanBL.GetInvoiceDetailsOnly(QuoComponent, IdKey, "");
                            }
                            else
                            {
                                dt_QuotationDetails = objSalesReturnBL.GetsalesInvoiceDetailsOnly(KeyVal, "Add");
                            }
                    ...
                    else if (Quote_Type == "TSI")
                    {
                        ...
                        if (!string.IsNullOrEmpty(IdKey))
                        {
                            if (IdKey != "ADD")
                            {
                                dt_QuotationDetails = objSalesReturnBL.GetTransitSalesInvoiceDetailsOnly(KeyVal, "Edit");
                                // dt_QuotationDetails = objPurchaseChallanBL.GetInvoiceDetailsOnly(QuoComponent, IdKey, "");
                            }
                            else
                            {
                                dt_QuotationDetails = objSalesReturnBL.GetTransitSalesInvoiceDetailsOnly(KeyVal, "Add");
                            }
                    ...
                    if (Convert.ToString(Request.QueryString["key"]) != "ADD")
                    {
                        ...
                        DataTable Transaction_dt = (DataTable)Session["RDEC_ReturnDetails"];


                        if (Transaction_dt != null && Transaction_dt.Rows.Count > 0)
                        {
                            foreach (DataRow dr in Transaction_dt.Rows)
                            {
                            ...
                            for (int i = 0; i < grid_Products.VisibleRowCount; i++)
                            {
                                ...
                                var Checkdt = Transaction_dt.Select("ComponentID='" + ComponentID + "' AND ProductID ='" + ProductID + "'");
                                if (Checkdt.Length > 0)
                                {
        ...
        {
            Boolean inUse = false;
            DataTable taxTable = (DataTable)Session["RDEC_FinalTaxRecord"];
            ...
            Session["RDEC_FinalTaxRecord"] = taxTable;

            return inUse;
        }


        public bool ImplementTaxonTaggingRDEC(int id, int slno)
        {
            Boolean inUse = false;
            DataTable taxTable = (DataTable)Session["RDEC_FinalTaxRecord"];
            ProcedureExecute proc = new ProcedureExecute("prc_taxReturnTable");
            proc.AddVarcharPara("@Module", 100, "RateDifferenceEntryCustomerReturn");
            proc.AddIntegerPara("@id", id);
            proc.AddIntegerPara("@slno", slno);
            proc.AddBooleanPara("@inUse", true, QueryParameterDirection.Output);
        ...
        public IEnumerable GetInvoiceInfo(DataTable SalesInvoicedt1, string Order_Id)
        {
            ...
            for (int i = 0; i < SalesInvoicedt1.Rows.Count; i++)
            {
                ...
                if (SelectedSalesInvoiceType == "SI")
                {
                ...
                else if (SelectedSalesInvoiceType == "TSI")
                {



                    if (ImplementTaxonTaggingRDEC(Convert.ToInt32(strQuoteDetails_Id), Convert.ToInt32(Orders.SrlNo)))
                    {
                        Orders.TaxAmount = "0.00";
                    }
                ...
                {
                    Session["RDEC_TaxDetails"] = tempMainTax;


                    Session["RDEC_InvoiceTaxDetails"] = "1";
                ...
                //}
                //else { Session["RDEC_TaxDetails"] = null; }
                //  main tax integrate with respect to first tag  invoice
                // warehouse
                ...
                if (tempWarehouse != null && tempWarehouse.Rows.Count > 0)
                {
                    ...
                    Session["RDEC_WarehouseData"] = tempWarehouse;
        ...
        public bool BindSessionByDatatable(DataTable dt)
        {
            ...
            Session["RDEC_ReturnDetails"] = SlsOreturnDT;
        ...
        protected void BindLookUp(string Customer, string SalesReturnDate, string branchId, string Type)
        {
            ...
            if (Type == "TSI")
            { ComponentTable = objSalesReturnBL.GetTransitInvoiceRDEC(Customer, SalesReturnDate, strReturnID, branchId); }
            else { ComponentTable = objSalesReturnBL.GetInvoiceRDEC(Customer, SalesReturnDate, strReturnID, branchId); }
        ...
        protected void acbpCrpUdf_Callback(object source, DevExpress.Web.CallbackEventArgsBase e)
        {
            ...
            if (Request.QueryString["key"] == "ADD")
            {
                if (reCat.isAllMandetoryDone((DataTable)Session["UdfDataOnAdd"], "USR") == false)
                {
                    ...
                    acbpCrpUdf.JSProperties["cpInvoiceTax"] = Session["RDEC_InvoiceTaxDetails"];
        ...
        public static object SetProjectCode(string OrderId, string TagDocType)
        {
            ...
            {
                ProcedureExecute proc = new ProcedureExecute("prc_CRMSalesReturn_Details");
                proc.AddVarcharPara("@Action", 500, "InvoicetaggingProjectdata");
                proc.AddVarcharPara("@SalesReturnID", 100, OrderId);
        ...
        public static object DocWiseSimilarProjectCheck(string quote_Id, string Doctype)
        {
            ...
            if (HttpContext.Current.Session["userid"] != null)
            {
                ...
                ProcedureExecute proc = new ProcedureExecute("prc_CRMSalesReturn_Details");
                proc.AddVarcharPara("@Action", 100, "GetProjectCheckForRDECInPInvoice");
        ...
        public static object taxUpdatePanel_Callback(string Action, string srl, string prodid)
        {
            ...
            try
            {
                ...
                if (Action == "DeleteAllTax")
                {
                    ...
                    DataTable taxDetails = (DataTable)HttpContext.Current.Session["RDEC_TaxDetails"];
                    ...
                    if (taxDetails != null)
                    {
                        ...
                        HttpContext.Current.Session["RDEC_TaxDetails"] = taxDetails;
                    }
                ...
                else if (Action == "DelProdbySl")
                {
                    ...
                    DataTable MainTaxDataTable = (DataTable)HttpContext.Current.Session["RDEC_FinalTaxRecord"];
                    ...
                    HttpContext.Current.Session["RDEC_FinalTaxRecord"] = MainTaxDataTable;
                    //GetStock(Convert.ToString(performpara.Split('~')[1]));
                    // DeleteWarehouse(Convert.ToString(performpara.Split('~')[1]));
                    DataTable taxDetails = (DataTable)HttpContext.Current.Session["RDEC_TaxDetails"];
                    if (taxDetails != null)
