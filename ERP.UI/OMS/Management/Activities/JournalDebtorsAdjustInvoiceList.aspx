...
<%@ Page Title="" Language="C#" MasterPageFile="~/OMS/MasterPage/ERP.Master" AutoEventWireup="true" CodeBehind="JournalDebtorsAdjustInvoiceList.aspx.cs" Inherits="ERP.OMS.Management.Activities.JournalDebtorsAdjustInvoiceList" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    <script>
        ...
        function AllControlInitilize() {
            if (isFirstTime) {
                ...
                if (localStorage.getItem('JournalDebtorsAdjustFromDate')) {
                    var fromdatearray = localStorage.getItem('JournalDebtorsAdjustFromDate').split('-');
                    var fromdate = new Date(fromdatearray[0], parseFloat(fromdatearray[1]) - 1, fromdatearray[2], 0, 0, 0, 0);
                    cFormDate.SetDate(fromdate);
                }

                if (localStorage.getItem('JournalDebtorsAdjustToDate')) {
                    var todatearray = localStorage.getItem('JournalDebtorsAdjustToDate').split('-');
                    var todate = new Date(todatearray[0], parseFloat(todatearray[1]) - 1, todatearray[2], 0, 0, 0, 0);
                    ctoDate.SetDate(todate);
                }
                if (localStorage.getItem('JournalDebtorsAdjustListBranch')) {
                    if (ccmbBranchfilter.FindItemByValue(localStorage.getItem('JournalDebtorsAdjustListBranch'))) {
                        ccmbBranchfilter.SetValue(localStorage.getItem('JournalDebtorsAdjustListBranch'));
                    }
        ...
        function OnViewClick(keyValue) {
            var url = 'JournalDebtorsAdjustmentInvoice.aspx?key=' + keyValue + '&req=V';
            window.location.href = url;
        }
        function onEditClick(id) {
            window.location.href = 'JournalDebtorsAdjustmentInvoice.aspx?Key=' + id;
        }
        ...
        function updateGridByDate() {
            ...
            if (cFormDate.GetDate() == null) {
                ...
                localStorage.setItem("JournalDebtorsAdjustFromDate", cFormDate.GetDate().format('yyyy-MM-dd'));
                localStorage.setItem("JournalDebtorsAdjustToDate", ctoDate.GetDate().format('yyyy-MM-dd'));
                localStorage.setItem("JournalDebtorsAdjustListBranch", ccmbBranchfilter.GetValue());
 ...
 //END REV 1.0
    ...
    <link href="CSS/JournalDebtorsAdjustInvoiceList.css" rel="stylesheet" />
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <div class="panel-heading">
        <div class="panel-title clearfix">
            <h3 class="pull-left">Adjustment of Documents - Journal with Sales Invoice</h3>
        </div>
    </div>
    <table class="padTab">
        <tr>
     ...
     <div id="spnDeleteLock" runat="server" style="display:none; color:red;text-align:center"></div>
        ...
        <div class="GridViewArea relative">
            ...
            <dx:LinqServerModeDataSource ID="EntityServerModeDataSource" runat="server" OnSelecting="EntityServerModeDataSource_Selecting"
                ContextTypeName="ERPDataClassesDataContext" TableName="v_JournalAdjustmentSalesInvoice" />
