...
{
    public partial class ProjectQuotation : System.Web.UI.Page
    {
            ...
            if (!IsPostBack)
            {
                ...
                if (Request.QueryString["key"] != null)
                {
                    if (Convert.ToString(Request.QueryString["key"]) != "ADD")
                    {
                        ...
                        lblHeadTitle.Text = "Modify Project Quotation/Proposal";
                        ...
                        {
                            lblHeadTitle.Text = "View Project Quotation/Proposal";
                    ...
                    else
                    {
                        ...
                        Session["TaxDetails"] = null;
                        CreateDataTaxTable();
                        lblHeadTitle.Text = "Add Project Quotation/Proposal";
        ...
        public void SetQuotationDetails()
        {
            ...
            if (QuotationEditdt != null && QuotationEditdt.Rows.Count > 0)
            {
                ...
                if (OrderDocIdsEditdt != null && OrderDocIdsEditdt.Rows.Count > 0)
                {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                Quotationdt.Rows.Add(Convert.ToString(i + 1), QuotationID, ProductID, Convert.ToString(dt.Rows[i]["Description"]),
                    Convert.ToString(dt.Rows[i]["Quantity"]), UOM, "", Convert.ToString(dt.Rows[i]["QuoteDetails_StockQty"]), UOM,
                               SalePrice, Discount, Amount, TaxAmount, TotalAmount, "I", ProductName, Quote_InlineRemarks, Order_Num1, Convert.ToString(dt.Rows[i]["InquiryID"]), DetailsId);
            }
        ...
        protected void grid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (IsDeleteFrom != "D")
            {
               ...
               .Select(g => g.Key);
                ...
                if (validate == "outrange" || validate == "duplicate" || validate == "checkWarehouse" || validate == "nullAmount" || validate == "errorUdf" || validate == "transporteMandatory" || validate == "nullStateCode" || validate == "TCMandatory" || validate == "BillingShippingNull" || validate == "duplicateProduct" || validate == "nullQuantity" || validate == "ExceedQuantity")
                {
        ...
        protected void grid_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            else if (strSplitCommand == "BindGridOnQuotation")
            {
                ...
                if (Quote_Nos != "$")
                {
                    ...
                    {
                        DataSet dt_Quotation = new DataSet();
                        ...
                        if (!string.IsNullOrEmpty(IdKey))
                        {
                            if (IdKey != "ADD")
                            {
                                dt_Quotation = objSlaesActivitiesBL.GetProjectInquiryDetailsFromSalesQuotation(QuoComponent1, QuoteDetails_Id, Product_id1, "Edit");
                                dt_QuotationDetails = dt_Quotation.Tables[0];
                            }
                            else
                            {
                                dt_Quotation = objSlaesActivitiesBL.GetProjectInquiryDetailsFromSalesQuotation(QuoComponent1, QuoteDetails_Id, Product_id1, "Add");
                                dt_QuotationDetails = dt_Quotation.Tables[0];
                            }
                        ...
                        Session["InlineRemarks"] = dt_Quotation.Tables[1];
                                    ...
                                    , ref string strQuoteNumber)
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("Prc_ProjectQuotation", con);
        ...
        protected void taxUpdatePanel_Callback(object source, DevExpress.Web.CallbackEventArgsBase e)
        {
            ...
            string performpara = e.Parameter;
            grid.JSProperties["cpProdbySlSet"] = "";
            if (performpara.Split('~')[0] == "DelProdbySl")
            {
                ...
                grid.JSProperties["cpProdbySlSet"] = "ProdbySlSet";
            }
        ...
        public void GetAllDropDownDetailForSalesQuotation(string userbranch)
        {
            ...
            DataTable Schemadt = objSlaesActivitiesBL.GetNumberingSchema(strCompanyID, userbranchHierarchy, FinYear, "112", "Y");
        ...
        protected void ComponentQuotation_Callback(object source, DevExpress.Web.CallbackEventArgsBase e)
        {
            ...
            if (e.Parameter.Split('~')[0] == "BindQuotationGrid")
            {
                ...
                QuotationTable = objBL.GetInquiryOnProjectQuotation(customer, OrderDate, status, Convert.ToInt32(ddl_Branch.SelectedValue), Convert.ToString(ddlInventory.SelectedValue));
        ...
        protected void cgridProducts_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            if (strSplitCommand == "BindProductsDetails")
            {
                ...
                if (Quote_Nos != "$")
                {
                    ...
                    {
                        dt_Quotation = objSlaesActivitiesBL.GetProjectInquiryDetailsFromSalesQuotation(QuoComponent, "", "", "");
                        dt_QuotationDetails = dt_Quotation.Tables[0];
                    }
                    Session["QuotationDetails"] = null;
        ...
        protected void BindLookUp(String customer, string OrderDate)
        {
            ...
            QuotationTable = objBL.GetInquiryOnProjectQuotation(customer, OrderDate, status,
    ...
    //public class ProductQuotationQuantity
    //{
    //    public Int64 productid { get; set; }
    //    public Int32 slno { get; set; }
    //    public Decimal Quantity { get; set; }
    ...
    //public class WaitingProjectProductQuantity
    //{
    //    public Int64 productid { get; set; }
    //    public Int32 pslno { get; set; }
    //    public Decimal pQuantity { get; set; }
