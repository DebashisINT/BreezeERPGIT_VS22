///================================================== Revision History =============================================
//1.0   Priti     V2.0.40      19-10-2023    0026911:Party Invoice No and Party Invoice Date required in the Document Search Window of the Grid for Adjustment of Documents - Journal
//====================================================== Revision History =============================================
        ...
        for (x in myObj) {
            ...
            for (key in myObj[0]) {
                ...
                if (PropertyCount == 0)
                    ...
                    txt += " <td class='hide'>" + myObj[x]["id"] + "</td>";
                else if (PropertyCount == 6 || PropertyCount == 7 || PropertyCount == 8 || PropertyCount == 9|| PropertyCount == 10 || PropertyCount == 11)
                    txt += " <td class='hide'>" + myObj[x]["AdvType"] + "</td>";
                else
                    txt += " <td style='width:220px'><input onclick='PopupTextClickJournal(event," + onSelect + ")' type='text' style='background-color: #3399520a;'" + UniqueIndex + "='" + count + "'onfocus='ReceiptGetFocus(event)'  onblur='ReceiptlostFocus(event)' onkeydown=ReceiptSelected(event,'" + UniqueIndex.toString() + "',event.target) width='100%'  readonly  />" + myObj[x][key] + "</td>";
                PropertyCount++;
...
function Vendorkeydown(e) {
    ...
    if (e.code == "Enter" || e.code == "NumpadEnter") {
        ...
        if ($("#txtVendSearch").val() != '') {
            ...
            callonServer("Services/Master.asmx/GetVendorForVendorPayRec", OtherDetails, "VendorTable", HeaderCaption, "VendorIndex", "SetCustomer");// Mantis ID  0019766
        }
    }
    else if (e.code == "ArrowDown") {
...
function LoadCustomerReceipt() {
    ...
    $.ajax({
        type: "POST",
        url: "/OMS/Management/Activities/Services/CustomerReceiptAdjustment.asmx/GetJournalCreditosVoucharList",
...
function showDocumentList() {
    ...
    $.ajax({
        type: "POST",
        url: "/OMS/Management/Activities/Services/CustomerReceiptAdjustment.asmx/GetPurchaseInvoiceDocumentList",
        data: JSON.stringify(OtherDetails),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (msg) {
...
function afterSave() {
    ...
    if (saveNewOrExit == 'N') {
        window.location.href = 'JournalCreditorsAdjustPInvoice.aspx?Key=Add';
        //DeleteAllRows();
        //DocumentList = [];
        //var numbSchm = cCmbScheme.GetValue();
        //var splitData = numbSchm.split('~');
        ...
        window.location.href = 'JournalCreditorsAdjustPInvoiceList.aspx';
    }
}
