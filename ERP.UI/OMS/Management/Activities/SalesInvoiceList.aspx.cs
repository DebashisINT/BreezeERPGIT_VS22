...
{
    public partial class SalesInvoiceList : ERP.OMS.ViewState_class.VSPage
    {
            ...
            if (i)
            {
                if (IsFilter == "Y")
                {
                    if (strBranchID == "0")
                    {
                        ...
                        var q = from d in dc.SalesInvoiceLists
        ...
        public static string GetEditablePermissionFromEInvoice(string SalesInvoiceID, string Action)
        {
            CRMSalesDtlBL objCRMSalesDtlBL = new CRMSalesDtlBL();
            string ispermission = "";
            ispermission = objCRMSalesDtlBL.EditablePermission(Convert.ToInt32(SalesInvoiceID), Action);
            return Convert.ToString(ispermission);
        }
        ...
        protected void WaitingInvoice_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            WaitingInvoice.DataBind();
        }

        protected void WaitingInvoice_DataBinding(object sender, EventArgs e)
        {
            DataTable dtResult = new DataTable();
            SqlConnection con = new SqlConnection(Convert.ToString(System.Web.HttpContext.Current.Session["ErpConnection"]));
            SqlCommand cmd = new SqlCommand("PRC_ServiceSchedule", con);
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("@action", "WaitingInvoiceList");
            ...
            WaitingInvoice.DataSource = dtResult;
        }
