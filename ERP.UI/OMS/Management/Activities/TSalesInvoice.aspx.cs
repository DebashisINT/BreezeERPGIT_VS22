    ...
    public partial class TSalesInvoice : VSPage //: System.Web.UI.Page
    {
      ...
      //  BusinessLogicLayer.DBEngine oDBEngine = new BusinessLogicLayer.DBEngine(ConfigurationManager.AppSettings["DBConnectionDefault"]);
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!String.IsNullOrEmpty(RequiredSIPParty))
            {
                if (RequiredSIPParty == "Yes")
                {
                    ...
                    ddl_TSalePosGst.ClientVisible = true;
                }
            ...
            if (!IsPostBack)
            {
                ...
                string BackDatedEntryPurchaseReturnManual = ComBL.GetSystemSettingsResult("BackDatedEntryTSI");
                ...
                //ddl_highSeasale.SelectedIndex = -1;
                ...
                if (Request.QueryString["key"] != null)
                {
                    if (Convert.ToString(Request.QueryString["key"]) != "ADD")
                    {
                        ...
                        if (hdnPageStatus.Value == "update")
                        {
                            ...
                            if (hdnPlaceShiptoParty.Value == "0")
                            {
                                ddl_TSalePosGst.ClientEnabled = false;
                            }
                            else
                            {
                                ddl_TSalePosGst.ClientEnabled = true;
                            }
                        }
                        ...
                        hdnPageEditId.Value = Convert.ToString(Request.QueryString["key"]);
                        ...
                        DataTable dt = oDBEngine.GetDataTable("Select 'Y' From tbl_trans_TransitSalesInvoice WHERE Invoice_TotalAmount<>UnPaidAmount AND Invoice_Id='" + strQuotationId + "'");
                        if (dt != null && dt.Rows.Count > 0)
                        {
                            lbl_quotestatusmsg.Text = "*** Document already used in another module.";
                            btn_SaveRecords.Visible = false;
                    ...
                    else
                    {
                        ...
                        dt2 = objemployeebal.GetSystemsettingmail("Show Email in Transit SI");
                        ...
                        if (Session["SI_SaveMode"] != null)  // it has been removed from coding side of Quotation list 
                        {
                            ...
                            {
                                ddl_numberingScheme.Value = Convert.ToString(Session["SI_schemavalue"]); // it has been removed from coding side of Quotation list 
                            }
                            if (Session["SaveNewValuesTSI"] != null)
                            {
                                List<string> SaveNewValues = (Session["SaveNewValuesTSI"]) as List<string>;
                                ddl_Branch.SelectedValue = SaveNewValues[0];
        ...
        protected void GridTCSdocs_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            proc.AddVarcharPara("@module_name", 500, "TSI");
        ...
        public void BindSystemSettings()
        {
           ...
           // BusinessLogicLayer.DBEngine objEngine = new BusinessLogicLayer.DBEngine(ConfigurationManager.AppSettings["DBConnectionDefault"]);
            ...
            DataTable DT = objEngine.GetDataTable("Config_SystemSettings", " Variable_Value ", " Variable_Name='TSalesInvoiceAutoPrint' AND IsActive=1");
            if (DT != null && DT.Rows.Count > 0)
            {
        ...
        public void SetInvoiceDetails(string strInvoiceId)
        {
            ...
            DataSet dsQuotationEditdt = objSalesInvoiceBL.ModifyOfInvoiceEditData(strInvoiceId);
            DataTable QuotationEditdt = dsQuotationEditdt.Tables[0];
            DataTable BillDEspatchTbl = dsQuotationEditdt.Tables[3];
            if (QuotationEditdt != null && QuotationEditdt.Rows.Count > 0)
            {
                ...
                string HighSeaSale = Convert.ToString(QuotationEditdt.Rows[0]["HighSeaSale"]);

                string StrSalesInvoiceID = Convert.ToString(QuotationEditdt.Rows[0]["SalesInvoiceID"]);
                string StrSalesInvoiceNo = Convert.ToString(QuotationEditdt.Rows[0]["SalesInvoiceNo"]);
                string StrSalesInvoiceDate = Convert.ToString(QuotationEditdt.Rows[0]["SalesInvoiceDate"]);
                string StrIsClose = Convert.ToString(QuotationEditdt.Rows[0]["IsClose"]);
                if (hdnBillDepatchsetting.Value == "1")
                {
                ...
                ddl_TSalePosGst.DataSource = dsQuotationEditdt.Tables[2];
                ddl_TSalePosGst.ValueField = "ID";
                ddl_TSalePosGst.TextField = "Name";
                ddl_TSalePosGst.DataBind();

                string PosForGst = Convert.ToString(QuotationEditdt.Rows[0]["PosForGst"]);


                ddl_TSalePosGst.Value = PosForGst;

                Sales_BillingShipping.SetBillingShippingTable(dsQuotationEditdt.Tables[1]);
                if (HighSeaSale.ToUpper() == "TRUE")
                {
                    ddl_highSeasale.SelectedValue = "1";
                }
                else if (HighSeaSale.ToUpper() == "FALSE")
                {
                    ddl_highSeasale.SelectedValue = "0";
                }
                else
                {
                ...
                {
                    rdl_SaleInvoice.SelectedValue = InvoiceCreatedFromDoc;
                ...
                if (!String.IsNullOrEmpty(StrSalesInvoiceID))
                {
                    string[] eachQuo = StrSalesInvoiceID.Split(',');
                    if (eachQuo.Length > 1)//More tha one quotation
                    {
                        //BindLookUp(Customer_Id, Quote_Date, InvoiceCreatedFromDoc, Branch_Id);
                        //foreach (string val in eachQuo)
                    ...
                    {
                        BindLookUp(Customer_Id, Quote_Date, "SO", Branch_Id);
                        gridRefSoNo.Value = StrSalesInvoiceID.Trim();
                        gridRefSoNo.Text = StrSalesInvoiceNo.Trim();
                        //foreach (string val in eachQuo)
                        //{
                        //    gridRefSoNo.GridView.Selection.SelectRowByKey(Convert.ToInt32(val));
                        //}
                    }
                    else//No Quotation selected
                    {
                ...
                txtRefSoDate.Text = StrSalesInvoiceDate.Trim();
            ...
            DataTable dtt = GetProjectEditData(strInvoiceId);
        ...
        protected void BindLookUp(string Customer, string OrderDate, string ComponentType, string BranchID)
        {
            ...
            if (Convert.ToString(lookup_Project.Value) != "" && Convert.ToString(lookup_Project.Value) != "0")
            {
                ComponentTable = objSalesInvoiceBL.GetComponentForTransitPurchaseInvoiceWithProject(Customer, OrderDate, ComponentType, FinYear, BranchID, Action, strInvoiceID, Convert.ToInt64(lookup_Project.Value));
            }
            else
            {
                 ComponentTable = objSalesInvoiceBL.GetComponentForTransitPurchaseInvoice(Customer, OrderDate, ComponentType, FinYear, BranchID, Action, strInvoiceID);
            }
            if (ComponentType == "TPB")
            {
            ...
            {
                gridRefSoNo.GridView.Selection.CancelSelection();
                gridRefSoNo.GridView.Selection.CancelSelection();
                gridRefSoNo.DataSource = ComponentTable;
                gridRefSoNo.DataBind();

                Session["SO_ComponentData"] = ComponentTable;
            }
        ...
        protected void grid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            {
                string InvoiceComponentDate = "", InvoiceComponent = "", SOComponentDate = "", SOComponentNo = "", SOComponentID = "";
                ...
                //List<object> SOList = gridRefSoNo.GridView.GetSelectedFieldValues("ComponentID");
                //foreach (object Quo in SOList)
                //{
                //    SOComponentID += "," + Quo;
                //}
                //SOComponentID = SOComponentID.TrimStart(',');

                SOComponentID = Convert.ToString(gridRefSoNo.Value);

                //List<object> SOListNO = gridRefSoNo.GridView.GetSelectedFieldValues("ComponentNumber");
                //foreach (object Quo in SOListNO)
                //{
                //    SOComponentNo += "," + Quo;
                //}
                //SOComponentNo = SOComponentNo.TrimStart(',');

                SOComponentNo = gridRefSoNo.Text;

                //string[] eachSO = SOComponentID.Split(',');
                //if (eachSO.Length == 1)
                //{
                //    SOComponentDate = Convert.ToString(txtRefSoDate.Text);
                //}
                //else
                //{
                //    SOComponentDate = "";
                ...
                string PosForGst = Convert.ToString(ddl_TSalePosGst.Value);
                ...
                if (hdnBillDepatchsetting.Value == "1")
                {
                    ...
                    // Rev Mantis Issue 24048 (31/05/2021)
                    //BillDespatchDtls["BillPinCode"] = (Convert.ToInt64(BtxtbillingPin.Text));
                    if (!string.IsNullOrEmpty(BtxtbillingPin.Text))
                    {
                        BillDespatchDtls["BillPinCode"] = (Convert.ToInt64(BtxtbillingPin.Text));
                    }
                    else
                    {
                        BillDespatchDtls["BillPinCode"] = Convert.ToInt64(0);
                    }
                    // End of Rev Mantis Issue 24048 (31/05/2021)
              ...
              //  BusinessLogicLayer.DBEngine objEngine = new BusinessLogicLayer.DBEngine(ConfigurationManager.AppSettings["DBConnectionDefault"]);
                ...
                if (ddl_highSeasale.SelectedItem.Value == "" || ddl_highSeasale.SelectedItem.Value == null)
                {
                    validate = "HighSeaSalesValue";
                }


                //object sumObject;
                ////sumObject = _tempQuotation.Compute("Sum(Amount)", string.Empty);
                //sumObject = tempQuotation.AsEnumerable()
                //    .Sum(x => Convert.ToDecimal(x["Amount"]));
                //object TotalsumObject;
                ////TotalsumObject = _tempQuotation.Compute("Sum(TotalAmount)", string.Empty);
                //TotalsumObject = tempQuotation.AsEnumerable()
                ...
                object sumTDSObject;
                //sumObject = _tempQuotation.Compute("Sum(Amount)", string.Empty);
                sumTDSObject = tempQuotation.AsEnumerable()
                    .Sum(x => Convert.ToDecimal(x["Amount"]));
                object TotalTDSsumObject;
                //TotalTDSsumObject = _tempQuotation.Compute("Sum(TotalAmount)", string.Empty);
                TotalTDSsumObject = tempQuotation.AsEnumerable()
                .Sum(x => Convert.ToDecimal(x["TotalAmount"]));

                DataTable dtTDS = new DataTable();
                ProcedureExecute proc1 = new ProcedureExecute("prc_TDSDetailsSI_Calc");
                proc1.AddVarcharPara("@VendorID", 500, hdnCustomerId.Value);
                proc1.AddVarcharPara("@invoice_id", 500, MainInvoiceID);
                proc1.AddVarcharPara("@Action", 500, "ShowTDSDetails");
                proc1.AddVarcharPara("@date", 500, dt_PLQuote.Text);
                proc1.AddVarcharPara("@totalAmount", 500, Convert.ToString(TotalTDSsumObject));
                proc1.AddVarcharPara("@taxableAmount", 500, Convert.ToString(sumTDSObject));
                proc1.AddVarcharPara("@branch_id", 500, strBranch);
                proc1.AddVarcharPara("@tds_code", 500, xtTDSSection.Value.ToString());
                dtTDS = proc1.GetTable();

                if (dtTDS != null && dtTDS.Rows.Count > 0)
                {
                    if (MainInvoiceID == "" && Convert.ToDecimal(dtTDS.Rows[0]["Amount"]) > 0 && Convert.ToDecimal(txtTDSAmount.Text) == 0)
                    {
                ...
                DataTable dtmandatory = objEngine.GetDataTable("Config_SystemSettings", " Variable_Value ", " Variable_Name='TInvoice_With_Order_Tagging' AND IsActive=1");
                if (dtmandatory != null && dtmandatory.Rows.Count > 0)
                {
                    ...
                    if (IsMandatory == "Yes")
                    {
                        if (Convert.ToString(gridRefSoNo.Value) == "")
                        {
                ...
                if (ddl_TSalePosGst.Value.ToString() == "S")
                {
                ...
                ProcedureExecute procT = new ProcedureExecute("prc_TransitSalesInvoice_Details");
                ...
                procT.AddVarcharPara("@HighSeaSale", 10, Convert.ToString(ddl_highSeasale.SelectedItem.Value));
                ...
                if (validate == "outrange" || validate == "duplicate" || validate == "TCSMandatory" || validate == "checkWarehouse" || validate == "duplicateProduct" || validate == "nullAmount" || validate == "nullQuantity" || validate == "transporteMandatory" || validate == "TCMandatory" || validate == "minSalePriceMust" || validate == "MRPLess" || validate == "OrderTaggingMandatory" || validate == "checkAcurateTaxAmount"
                  || validate == "TDSMandatory")
                {
                ...
                else
                {
                    ...
                    tSalesInvoiceNumber.Value = UniqueQuotation;

                    #region To Show By Default Cursor after SAVE AND NEW
                    if (Convert.ToString(Session["TSI_ActionType"]) == "Add") // session has been removed from quotation list page working good
                    {

                        List<string> myList = new List<string> { ddl_Branch.SelectedValue };
                        Session["SaveNewValuesTSI"] = myList;
                        ...
                        schemaid = Convert.ToString(ddl_numberingScheme.Value).Split('~');
                    ...
                    ModifyQuatation(strIsInventory, MainInvoiceID, strSchemeType, UniqueQuotation, strInvoiceDate, strCustomer, strContactName,
                                    ...
                                    approveStatus, ActionType, PosForGst, ref strIsComplete, ref strQuoteID, partyInvNo, partyInvDate, partyorderNo, partyorderDate, ref ReturnLedgerAmt
                                    , IsClose, SOComponentDate, SOComponentNo, SOComponentID, Proj_id, strTCScode, strTCSappl
                                    , strTCSpercentage, strTCSamout, drdTransCategory.SelectedValue, _ReverseCharge, BillDespatch
                    ...
                    if (strIsComplete == 1)
                    {
                        DataTable dts = oDBEngine.GetDataTable("select ISNULL(CONVERT (Decimal(18,2) , Invoice_TotalAmount),0) as TotalAmt,ISNULL(Irn,'') IRN from tbl_trans_TransitSalesInvoice where Invoice_Id=" + Convert.ToInt64(strQuoteID) + "");
                        ...
                        if (ActionType == "Add")
                        {
                            ...
                            hdnTInvId.Value = Convert.ToString(strQuoteID);
                        }
                        else
                        {
                            grid.JSProperties["cpQuotationID"] = "";
        ...
        protected void grid_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            else if (strSplitCommand == "BindGridOnQuotation")
            {
                ...
                // string strType = Convert.ToString("TPB");
                ...
                if (rdl_SaleInvoice.SelectedValue == "TPB")
                {
                    lookup_Project.ClientEnabled = false;
                }

                Session["TSI_WarehouseData"] = GetTaggingWarehouseData(ComponentDetailsIDs, strType);
                if (strType != "TPB") Session["TSI_FinalTaxRecord"] = GetComponentEditedTaxData(ComponentDetailsIDs, strType);
        ...
        public void ModifyQuatation(string strIsInventory, string QuotationID, string strSchemeType, string strQuoteNo, string strQuoteDate, string strCustomer, string strContactName,
                                    ...
                                    DataTable Productdt, DataTable TaxDetailTable, DataTable Warehousedt, DataTable InvoiceTaxdt, DataTable BillAddressdt, string approveStatus, string ActionType, string PosForGst,
                                    ...
                                    , string IsClose, string SOComponentDate, string SOComponentNo, string SOComponentID, Int64 Proj_id, string strTCScode,
            string strTCSappl, string strTCSpercentage, string strTCSamout, string TransacCategory, Boolean _ReverseCharge, DataTable BillDespatch
            , string strTDScode, string strTDSappl, string strTDSpercentage, string strTDSamout)
        {
            try
            {
              ...
              //  SqlConnection con = new SqlConnection(ConfigurationManager.AppSettings["DBConnectionDefault"]);
                ...
                {
                    string cdate = "";
                    if (strComponenyType == "TPB")
                    {
                        cdate = strInvoiceComponentDate.Substring(3, 2) + "-" + strInvoiceComponentDate.Substring(0, 2) + "-" + strInvoiceComponentDate.Substring(6, 4);
                    }
                    else if (strComponenyType == "SO")
                    {
                        cdate = Convert.ToDateTime(strInvoiceComponentDate).ToString("MM-dd-yyyy");
                    }
                ...
                bool highseasale = false;
                if (ddl_highSeasale.SelectedItem.Value == "1")
                {
                    highseasale = true;
                }
                else if (ddl_highSeasale.SelectedItem.Value == "0")
                {
                    highseasale = false;
                }
                else
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "Notify", "jAlert('High Sea Sales is mandatory');", true);
                    return;
                }
                ...
                cmd.Parameters.AddWithValue("@SOComponentID", SOComponentID);
                cmd.Parameters.AddWithValue("@SOComponentNo", SOComponentNo);
                ...
                if (SOComponentDate != "")
                {
                    ...
                    cSOdate = Convert.ToDateTime(SOComponentDate).ToString("MM-dd-yyyy");


                    DateTime d2;
                    d2 = DateTime.ParseExact(cSOdate, "MM-dd-yyyy", null);
                    cmd.Parameters.AddWithValue("@SOComponentDate", d2);
                }
                ...
                if (strInvoiceID > 0)
                {
                    if (chkSendMail.Checked)
                    {
                        if (Convert.ToString(Request.QueryString["key"]) == "ADD")
                        {
                            ...
                            exportToPDF.ExportToPdfforEmail("TransitSalesInvoice~D", "TSInvoice", Server.MapPath("~"), "1", Convert.ToString(strInvoiceID));
        ...
        {
            DataTable dt = new DataTable();
            if (HttpContext.Current.Session["TSI_FinalTaxRecord"] != null)
            {
                dt = (DataTable)HttpContext.Current.Session["TSI_FinalTaxRecord"];
                dt.Rows.Clear();
                //HttpContext.Current.Session["FinalTaxRecord"]=null;
                HttpContext.Current.Session["TSI_FinalTaxRecord"] = dt;
            }
        ...
        {
            DataTable Warehousedt = new DataTable();
            if (HttpContext.Current.Session["TSI_WarehouseData"] != null)
            {
                Warehousedt = (DataTable)HttpContext.Current.Session["TSI_WarehouseData"];
                ...
                HttpContext.Current.Session["TSI_WarehouseData"] = Warehousedt;
            }
        }
        ...
        //public  static void GetStockByAjax(string strProductID)
        //{
        //    BusinessLogicLayer.DBEngine oDBEngine = new BusinessLogicLayer.DBEngine();
        //    string strBranch = Convert.ToString(ddl_Branch.);
        //    acpAvailableStock.JSProperties["cpstock"] = "0.00";
         ...
         public static void CreateDataTaxTableByAjax()
        {
            ...
            TaxRecord.Columns.Add("Percentage", typeof(System.Decimal));
            TaxRecord.Columns.Add("Amount", typeof(System.Decimal));
            HttpContext.Current.Session["TSI_FinalTaxRecord"] = TaxRecord;
        }
        ...
        protected void aspxGridTax_CellEditorInitialize(object sender, DevExpress.Web.ASPxGridViewEditorEventArgs e)
        {
            ...
            //{
            //    e.Editor.ReadOnly = false;
            //    var textBoxProperties = e.Column.PropertiesEdit as DevExpress.Web.TextBoxProperties;
            //    textBoxProperties.MaskSettings.Mask = "<0..99999g>.<00..99>";
            //    textBoxProperties.DisplayFormatString = "0.00";
            //    textBoxProperties.MaskSettings.IncludeLiterals = MaskIncludeLiteralsMode.None;
            //    textBoxProperties.DisplayFormatInEditMode = true;
        ...
        protected void PopulateGSTCSTVAT(string type)
        {
            ...
            if (type == "2")
            {
                ...
                if (ddl_TSalePosGst.Value.ToString() == "S")
                {
                    sstateCode = Convert.ToString(Sales_BillingShipping.GetShippingStateId());
                }
                else
                {
                    sstateCode = Convert.ToString(Sales_BillingShipping.GetBillingStateId());
                }
                ShippingState = sstateCode;
                if (ShippingState.Trim() != "")
                {
        ...
        public void GetAllDropDownDetailForSalesQuotation(string userbranch)
        {
            ...
            {
                ddl_numberingScheme.TextField = "SchemaName";
                ddl_numberingScheme.ValueField = "Id";
        ...
        protected void cgridProducts_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            if (strSplitCommand == "BindProductsDetails")
            {
                ...
                if (Quote_Nos != "$")
                {
                    ...
                    {
                        strAction = "GetSalesOrderProducts";
                    }
        ...
        #region Ref. Sales Order

        protected void CallbackPanelSO_Callback(object source, DevExpress.Web.CallbackEventArgsBase e)
        {
            string status = string.Empty;
            string Customer = string.Empty;
            string OrderDate = string.Empty;
            ...
            if (e.Parameter.Split('~')[0] == "BindRefSoNoGrid")
            {
                BranchID = Convert.ToString(ddl_Branch.SelectedValue);
                if (e.Parameter.Split('~')[1] != null) Customer = e.Parameter.Split('~')[1];
                if (e.Parameter.Split('~')[2] != null) OrderDate = e.Parameter.Split('~')[2];
                ...
                if (e.Parameter.Split('~')[3] == "DateCheck")
                {
                    gridRefSoNo.GridView.Selection.UnselectAll();
                }
                Int64 ProjectValue=0;
                if(e.Parameter.Split('~')[5] != null && e.Parameter.Split('~')[5] !="" && e.Parameter.Split('~')[5] !="0")
                {
                    ProjectValue = Convert.ToInt64(e.Parameter.Split('~')[5]);
                }
               ...
               // DataTable ComponentTable = objSalesInvoiceBL.GetComponentForTransitPurchaseInvoice(Customer, OrderDate, ComponentType, FinYear, BranchID, Action, strInvoiceID);
                ...
                {
                    ComponentTable = objSalesInvoiceBL.GetComponentForTransitPurchaseInvoiceWithProject(Customer, OrderDate, ComponentType, FinYear, BranchID, Action, strInvoiceID, ProjectValue);
                }
                else
                {
                    ComponentTable = objSalesInvoiceBL.GetComponentForTransitPurchaseInvoice(Customer, OrderDate, ComponentType, FinYear, BranchID, Action, strInvoiceID);
        ...
        protected void gridRefSoNo_DataBinding(object sender, EventArgs e)
        {
            if (Session["SO_ComponentData"] != null)
            {
                gridRefSoNo.DataSource = (DataTable)Session["SO_ComponentData"];
            }
        }
        protected void DatePanelSODate_Callback(object sender, CallbackEventArgsBase e)
        {
            string strSplitCommand = e.Parameter.Split('~')[0];
            if (strSplitCommand == "BindSODate")
            {
                string ComponentNo = Convert.ToString(e.Parameter.Split('~')[1]);
                string type = Convert.ToString(e.Parameter.Split('~')[2]);
                ...
                if (dtDetails != null && dtDetails.Rows.Count > 0)
                {
                    ...
                    if (!string.IsNullOrEmpty(Date))
                    {
                        txtRefSoDate.Text = Convert.ToString(Date);
                    }
                }
            }
        }
        ...
        protected void SelectPanel_Callback(object sender, CallbackEventArgsBase e)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    string reportName = Convert.ToString("TransitSalesInvoice~D");
                    string NoofCopy = "";
        ...
        public int SendMail(string Output)
        {
            ...
            if (Convert.ToString(dt2.Rows[0]["Variable_Value"]) == "Yes")
            {
                ...
                if (ConfigurationManager.AppSettings["IsDevelopedZone"] != null)
                {
                    FilePath = Server.MapPath("~/Reports/Reports/RepxReportDesign/SalesInvoice/DocDesign/PDFFiles/" + "TransitSalesInvoice-Original-" + Output + ".pdf");
                }
                else
                {
                    FilePath = Server.MapPath("~/Reports/RepxReportDesign/SalesInvoice/DocDesign/PDFFiles/" + "TransitSalesInvoice-Original-" + Output + ".pdf");
                }
                ...
                if (dt_EmailConfig.Rows.Count > 0)
                {
                    ...
                    dt_Emailbodysubject = objemployeebal.Getemailtemplates("18");
                    ...
                    if (dt_Emailbodysubject.Rows.Count > 0)
                    {
                        ...
                        dt_EmailConfigpurchase = objemployeebal.Getemailtagsforpurchase(Output, "TransitSalesInvoiceEmailTags");
        ...
        protected void EntityServerModeDataTSI_Selecting(object sender, DevExpress.Data.Linq.LinqServerModeDataSourceSelectEventArgs e)
        {
            ...
            if(Convert.ToString(Request.QueryString["key"]) != "ADD")
            {
                string strInvoiceId = Convert.ToString(Request.QueryString["key"]);
                DataTable dt = new DataTable();
                ProcedureExecute proc = new ProcedureExecute("prc_TransitSalesInvoice_Details");
                proc.AddVarcharPara("@Action", 500, "ProjectEditBranchdata");
                proc.AddVarcharPara("@DocId", 200, strInvoiceId);
                dt = proc.GetTable();
                hdnCustomerId.Value = Convert.ToString(dt.Rows[0]["Customer_Id"]);
                string Branchid = Convert.ToString(dt.Rows[0]["Invoice_BranchId"]);
        ...
        {
            List<EInvDEtailsTSI> Detail = new List<EInvDEtailsTSI>();
            if (HttpContext.Current.Session["userid"] != null)
            {
        ...
        [WebMethod]
        public static object SetProjectCode(string OrderId)
        {
            List<DocumentDetails> Detail = new List<DocumentDetails>();
            if (HttpContext.Current.Session["userid"] != null)
            {
                ProcedureExecute proc = new ProcedureExecute("prc_TransitSalesInvoice_Details");
                proc.AddVarcharPara("@Action", 500, "TPurchaseInvoicetaggingProjectdata");
                proc.AddVarcharPara("@InvoiceID", 100, OrderId);
        ...
        public static object CreditDaysCheckchanged(string key, string startDate)
        {
            TransitSalesInvoiceBL objSalesInvoiceBL = new TransitSalesInvoiceBL();
            string customerduedate = string.Empty;
            DataTable dtDeuDate = objSalesInvoiceBL.GetCustomerDetails_InvoiceRelated(key);
            string creditdays = Convert.ToString(dtDeuDate.Rows[0]["CreditDays"]);

            string strInvoiceDate = Convert.ToString(startDate);
            if (strInvoiceDate != null && strInvoiceDate != "")
            {
                DateTime InvoiceDate = Convert.ToDateTime(strInvoiceDate);
                DateTime duedate = InvoiceDate.AddDays(Convert.ToInt32(creditdays));
            ...
            return customerduedate + '@' + creditdays;
        }


        [WebMethod(EnableSession = true)]
        public static object BindComponentGrid(string key, string startDate, string type, string branchid)
        {
            TransitSalesInvoiceBL objSalesInvoiceBL = new TransitSalesInvoiceBL();
            string status = string.Empty;
            string Customer = string.Empty;
            ...
            BranchID = branchid;
            //BranchID = Convert.ToString(branchid);
            //if (e.Parameter.Split('~')[1] != null) Customer = e.Parameter.Split('~')[1];
            //if (e.Parameter.Split('~')[2] != null) OrderDate = e.Parameter.Split('~')[2];
            //if (e.Parameter.Split('~')[4] != null) ComponentType = e.Parameter.Split('~')[4];
            //if (e.Parameter.Split('~')[3] == "DateCheck")
            ...
            string strInvoiceID = Convert.ToString(HttpContext.Current.Session["TSI_InvoiceID"]);
            DataTable ComponentTable = objSalesInvoiceBL.GetComponentForTransitPurchaseInvoice(key, startDate, type, FinYear, BranchID, Action, strInvoiceID);
            ...
            HttpContext.Current.Session["TSI_ComponentData"] = ComponentTable;
            return null;
        }
        ...
        public static object taxUpdatePanel_Callback(string Action, string srl, string prodid)
        {
            ...
            try
            {
                ...
                {
                    DataTable MainTaxDataTable = (DataTable)HttpContext.Current.Session["TSI_FinalTaxRecord"];
                    ...
                    HttpContext.Current.Session["TSI_FinalTaxRecord"] = MainTaxDataTable;
                   // GetStock(Convert.ToString(srl));
                    DeleteWarehouseByAjax(Convert.ToString(srl));
                    DataTable taxDetails = (DataTable)HttpContext.Current.Session["TSI_TaxDetails"];
                    if (taxDetails != null)
                    {
                        foreach (DataRow dr in taxDetails.Rows)
                        {
                        ...
                        HttpContext.Current.Session["TSI_TaxDetails"] = taxDetails;
                    }
                }
        ...
        // Mantis Issue 24527 [ parameter string tds_code added ]
        [WebMethod(EnableSession = true)]
        public static object getTDSDetails(string CustomerId, string invoice_id, string date, string totalAmount, string taxableAmount, string branch_id, string tds_code)
        {
            ...
            proc.AddVarcharPara("@Action", 500, "ShowTSITDSDetails");
