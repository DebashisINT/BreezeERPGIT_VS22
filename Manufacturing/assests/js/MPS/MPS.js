...
$(document).ready(function () {   
    ...
    var column2 = gridEstimateProductEntryList.GetColumnByField("ProductName");
    PrdProdNameIndex = column2.index;

    var column3 = gridEstimateProductEntryList.GetColumnByField("ProductDescription");
    PrdProdDescIndex = column3.index;

    var column4 = gridEstimateProductEntryList.GetColumnByField("ProductQty");
    PrdProdQtyIndex = column4.index;

    var column5 = gridEstimateProductEntryList.GetColumnByField("ProductUOM");
    PrdProdUOMIndex = column5.index;
    ...
    var column7 = gridEstimateProductEntryList.GetColumnByField("Amount");
    PrdAmountIndex = column7.index;

    

    

    var column8 = gridEstimateProductEntryList.GetColumnByField("Remarks");
    PrdRemarksIndex = column8.index;
...
function GridNonInventoryProductlist(SearchKey, type, txtid) {
    if (SearchKey != "") {
        ...
        callonServer("../Models/pManufacturing_WebServiceList.asmx/GetMPSProductDetailsList", OtherDetails, "GridProductTable", HeaderCaption, "NonIProductIndex", "SetGridNonInventoryProduct");
    }
...
function GetEstimateList(SearchKey, slno) {
    ...
    callonServer("../Models/pManufacturing_WebServiceList.asmx/GetBOMList", OtherDetails, "GridEstimateTable", HeaderCaption, "EstimateIndex", "SetGridEstimateProduct");
}
...
function GridProductlist(SearchKey, type, txtid) {
    ...
    if (SearchKey != "") {
        ...
        callonServer("../Models/pManufacturing_WebServiceList.asmx/GetMPSProductDetailsList", OtherDetails, "GridProductTable", HeaderCaption, "GridProductIndex", "SetGridProduct");
    }
...
function OpenOrderList(s, e) {
    ...
    var txt = "<table  class='dynamicPopupTbl' width=\"100%\"><tr class=\"HeaderStyle\"><th class='hide'>id</th><th>Order No.</th><th>Order Date</th></tr><table>";
    $("#txtGridOrderName").val("");
    document.getElementById("OrderTable").innerHTML = txt;
    $('#GridOrderlistModel').modal('show');
    $('#txtGridOrderName').focus();
    setTimeout(function () {
        $('#txtGridOrderName').focus();
    }, 200);
...
function OrderKeyDown(s, e) {
    ...
    if (e.htmlEvent.key == "Enter") {       
        ...
        if ($("input[OrderIndex=0]"))
            $("input[OrderIndex=0]").focus();
    }
}

function OrderListkeydown(e) {

    if (e.code == "Enter" || e.code == "NumpadEnter") {
        if ($("#txtGridOrderName").val().trim() != '') {
            GetOrderlist($("#txtGridOrderName").val());
            //gridproductlist = 1;
        }
        else {
            var txt = "<table  class='dynamicPopupTbl' width=\"100%\"><tr class=\"HeaderStyle\"><th class='hide'>id</th><th>Order No.</th><th>Order Date</th></tr><table>";
...
function GetOrderlist(SearchKey) {
    //debugger;
    ...
    if (SearchKey != "") {
       // gridproductlist = 1;
        ...
        HeaderCaption.push("Order Date");
        
        callonServer("../Models/pManufacturing_WebServiceList.asmx/GetMPSOrderDetailsList", OtherDetails, "OrderTable", HeaderCaption, "OrderIndex", "SetOrder");
    }
...
//SetGridProduct
function SetOrder(Id, Name, e) {    
    var OrderID = Id;
    var OrderName = Name;
    if (OrderID != "") {
        btnOrder.SetText(OrderName);
        $("#hdnOrderID").val(OrderID);
    }
    $('#GridOrderlistModel').modal('hide');
}
...
function ValueSelected(e, indexName) {
    ...
    if (e.code == "Enter") {
        ...
        if (Id) {
            ...
            else if (indexName == "OrderIndex") {
                SetOrder(Id, name, null);
            }
...
function Customerkeydown(e) {
    ...
    if (e.code == "Enter" || e.code == "NumpadEnter") {
        ...
        if ($("#txtCustSearch").val() != "") {
            callonServer("../Models/pManufacturing_WebServiceList.asmx/GetCustomer", OtherDetails, "CustomerTable", HeaderCaption, "customerIndex", "SetCustomer");
        }
    }
    else if (e.code == "ArrowDown") {
